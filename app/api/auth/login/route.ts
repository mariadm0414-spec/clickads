...
export async function POST(req: Request) {
    try {
        ...
        const { data: allMatches, error: dbError } = await supabaseAdmin
        ...
        const userData = allMatches && allMatches.length > 0 ? allMatches[0] : null;

        if (!userData) {
            console.log(`Access denied: ${cleanEmail} not found. Matches count: ${allMatches?.length || 0}`);
