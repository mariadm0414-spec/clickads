...
import { withRetry, RetryableHttpError, parseRetryAfterMs } from "@/app/lib/retry";
...
// --- Plantillas visuales de creativos (antes vivían dentro de handleGenerate y se
// recreaban en cada llamada; se sacan a nivel de módulo para reusarlas también en
// la generación por lotes sin duplicar el arreglo). ---
interface AdTypeGoalContext {
    outputLang: string;
    productName?: string;
...
export interface AdType {
    ...
    name: string;
    style: string;
    goal: (ctx: AdTypeGoalContext) => string;
}

export const AD_TYPES: AdType[] = [
    { id: "TESTIMONIAL", name: "TESTIMONIAL", style: "Cinematic professional product photography with dramatic commercial lighting.", goal: ({ outputLang }) => `Añadir 2-3 burbujas de testimonios elegantes con FOTOGRAFÍAS REALES DE PERSONAS. Texto corto en ${outputLang}.` },
    { id: "SALES_CTA", name: "SALES_CTA", style: "Urban premium lifestyle editorial photography.", goal: ({ outputLang }) => `Incluir un titular impactante en ${outputLang} relacionado con el producto. Añadir un botón flotante moderno.` },
    { id: "BENEFITS", name: "BENEFITS", style: "Clean luxury minimalist showroom.", goal: ({ outputLang, productName, targetAudience }) => `Resaltar 3 beneficios clave del producto "${productName || 'de la imagen'}" para el público "${targetAudience || 'general'}" usando iconos minimalistas en ${outputLang}.` },
    { id: "INFOGRAPHIC", name: "INFOGRAPHIC", style: "Flat-lay professional editorial layout.", goal: ({ outputLang }) => `Crear una infografía premium. Señalar 3-4 características REALES. Texto breve en ${outputLang}.` },
    ...
    { id: "HERO", name: "HERO", style: "Premium high-energy lifestyle advertising photography.", goal: ({ outputLang }) => `Diseño 'HERO': Titular GIGANTE en ${outputLang}, 3 beneficios con checks, badges de "ENVÍO SEGURO", y persona feliz.` },
...
// --- Ganchos de copy (dolor, beneficio, urgencia, prueba social, curiosidad, escasez).
// Se combinan con AD_TYPES para diversificar los creativos de un mismo lote: cada
// creativo recibe una plantilla visual + un gancho distinto, no solo una copia
// cosmética del mismo ángulo. ---
...
export const HOOKS: Hook[] = [
    { id: "DOLOR", label: "Dolor", instruction: "Parte de un dolor o frustración muy específica del público objetivo y muestra cómo el producto la elimina. El mensaje debe nombrar el problema antes de resolverlo." },
    { id: "BENEFICIO", label: "Beneficio", instruction: "Enfócate en el beneficio principal y la transformación positiva que logra el producto. Tono aspiracional, sin mencionar el problema." },
    { id: "URGENCIA", label: "Urgencia", instruction: "Genera una sensación real de urgencia (tiempo limitado, oferta por vencer) que empuje a actuar de inmediato." },
    { id: "PRUEBA_SOCIAL", label: "Prueba Social", instruction: "Apóyate en prueba social: testimonios, número de clientes satisfechos o reseñas, para generar confianza." },
    { id: "CURIOSIDAD", label: "Curiosidad", instruction: "Abre con una pregunta intrigante o un dato poco conocido sobre el producto que genere curiosidad." },
    { id: "ESCASEZ", label: "Escasez", instruction: "Comunica escasez o exclusividad: edición limitada, disponibilidad reducida, algo que no todos pueden tener." }
];
...
/**
 * Arma `count` combinaciones (plantilla visual + gancho de copy) para un lote.
 * Mezcla todas las combinaciones posibles (AD_TYPES × HOOKS) y toma las primeras
 * `count` sin repetir; si `count` supera el total de combinaciones posibles,
 * vuelve a mezclar y continúa, evitando repetir la combinación inmediatamente
 * anterior en la costura entre pasadas.
 */
export function buildBatchCombos(count: number): CreativeCombo[] {
    const basePairs: CreativeCombo[] = [];
    for (const adType of AD_TYPES) {
        for (const hook of HOOKS) {
            basePairs.push({ adType, hook });
        }
    }

    const result: CreativeCombo[] = [];
    let lastKey = "";
    while (result.length < count) {
        const pass = shuffle(basePairs);
        if (pass.length > 1 && `${pass[0].adType.id}|${pass[0].hook!.id}` === lastKey) {
            [pass[0], pass[1]] = [pass[1], pass[0]];
        }
        for (const combo of pass) {
            if (result.length >= count) break;
            result.push(combo);
            lastKey = `${combo.adType.id}|${combo.hook!.id}`;
        }
    }
    return result;
}

const BATCH_COUNT_OPTIONS = [1, 5, 10, 100] as const;
const BATCH_CONCURRENCY = 4;

// --- UGC Image Generator: parámetros avanzados del modelo/escena/fotografía. ---
// Mismo patrón de "avatar" ya usado en el generador de reseñas de WhatsApp
// (waReviewAvatarEthnicity / waReviewAvatarCountry) aplicado aquí también.
export const UGC_WHO_OPTIONS = ["Mujer", "Hombre", "Pareja", "Familia", "Adulto mayor", "Deportista", "Mamá", "Influencer"];
export const UGC_ETHNICITIES = ["Latina", "Caucásica", "Afrodescendiente", "Asiática", "Medio Oriente", "Mixta / Multiétnica"];
export const UGC_COUNTRIES = ["Colombia", "México", "España", "Argentina", "Chile", "Perú", "Estados Unidos (Hispano)", "Genérico / Internacional"];
export const UGC_BODY_TYPES = ["Delgada", "Atlética / Fit", "Promedio", "Curvy", "Plus Size"];
export const UGC_HAIR_COLORS = ["Castaño", "Rubio", "Negro", "Pelirrojo", "Canoso / Gris"];
export const UGC_HAIR_STYLES = ["Liso", "Ondulado", "Rizado", "Corto", "Largo", "Recogido"];
export const UGC_TIME_OF_DAY = ["Día (luz natural)", "Atardecer (Golden Hour)", "Noche (luz artificial)", "Interior luz cálida", "Nublado (luz suave)"];
export const UGC_OUTFITS = ["Casual", "Deportivo / Athleisure", "Elegante", "Playa / Verano", "Pijama / Loungewear", "Oficina / Formal"];
export const UGC_CAMERA_ANGLES = ["Primer plano (rostro)", "Plano medio", "Cuerpo completo", "Plano cenital (desde arriba)", "Ángulo bajo"];
export const UGC_LIGHTING = ["Luz natural de ventana", "Golden hour", "Ring light", "Flash directo (espontáneo)", "Nublado / luz suave", "Interior cálido"];
export const UGC_FILTERS = ["Ninguno (natural)", "Grano sutil (film grain)", "Filtro cálido Instagram", "Blanco y negro editorial", "Alto contraste VSCO"];

export interface UgcRealismLevel {
    id: string;
    label: string;
    instruction: string;
}

export const UGC_REALISM_LEVELS: UgcRealismLevel[] = [
    { id: "ultra", label: "Ultra realista (imperfecciones naturales)", instruction: "The skin must show natural texture, visible pores, and subtle imperfections — avoid airbrushed, plastic, or overly smooth AI-generated skin. Slight natural asymmetries and everyday styling are expected, not a flawless studio look." },
    { id: "balanced", label: "Balanceado", instruction: "The look should be realistic and natural but well-groomed, similar to a well-taken everyday photo shared on social media." },
    { id: "polished", label: "Pulido / Editorial", instruction: "The look can be slightly more polished and aspirational, like a well-curated influencer photo, while still looking like a real, authentic person and not a studio render." }
];

// --- Video UGC (Veo) ---
// Un solo tier (Fast) por decisión del usuario: más barato y menos superficie de UI.
// Precio triangulado desde fuentes secundarias (no verificado 1:1 contra la tabla oficial) — mostrar siempre como estimado.
export const VEO_MODEL = "veo-3.1-fast-generate-preview";
export const VEO_PRICE_PER_SECOND_USD = 0.15;
export const VEO_CLIP_DURATION_SECONDS = 8;
export const VEO_ESTIMATED_COST_USD = (VEO_PRICE_PER_SECOND_USD * VEO_CLIP_DURATION_SECONDS).toFixed(2);
...
interface Project {
    ...
    videoResults?: {
        ...
        // Ruta a /api/veo/video-proxy (NO una URL de storage persistente) — deja de funcionar
        // cuando el enlace original de Google expira, ~48h después de generado.
        videoUrl?: string;
        errorMessage?: string;
        createdAt: number;
    }[];
...
// Bloque colapsable reutilizado por las 4 secciones de parámetros del UGC Image
// Generator (Modelo / Escena / Fotografía / Extra). Mismo look que el resto de
// bloques de "glass-card" del panel, solo con header clickeable + chevron.
function UgcSection({ title, icon, isOpen, onToggle, children }: { title: string; icon?: React.ReactNode; isOpen: boolean; onToggle: () => void; children: React.ReactNode }) {
    return (
...
export default function Dashboard() {
    ...
    // UGC Image Generator — parámetros avanzados (modelo/escena/fotografía/extra)
    const [ugcEthnicity, setUgcEthnicity] = useState(UGC_ETHNICITIES[0]);
    const [ugcCountry, setUgcCountry] = useState(UGC_COUNTRIES[0]);
    const [ugcBodyType, setUgcBodyType] = useState("Promedio");
    const [ugcHairColor, setUgcHairColor] = useState(UGC_HAIR_COLORS[0]);
    const [ugcHairStyle, setUgcHairStyle] = useState(UGC_HAIR_STYLES[0]);
    const [ugcTimeOfDay, setUgcTimeOfDay] = useState(UGC_TIME_OF_DAY[0]);
    const [ugcOutfit, setUgcOutfit] = useState(UGC_OUTFITS[0]);
    const [ugcCameraAngle, setUgcCameraAngle] = useState("Plano medio");
    const [ugcLighting, setUgcLighting] = useState(UGC_LIGHTING[0]);
    const [ugcFilter, setUgcFilter] = useState(UGC_FILTERS[0]);
    const [ugcRealism, setUgcRealism] = useState(UGC_REALISM_LEVELS[0].id);
    const [ugcExtraInstructions, setUgcExtraInstructions] = useState("");
    const [ugcOpenSections, setUgcOpenSections] = useState<{ modelo: boolean; escena: boolean; fotografia: boolean; extra: boolean }>({ modelo: true, escena: false, fotografia: false, extra: false });

    // Fotos de Reseña — foto de producto en mano/sobre superficie + texto de reseña
    const [reviewPhotoProductImage, setReviewPhotoProductImage] = useState<string | null>(null);
    const [reviewPhotoProduct, setReviewPhotoProduct] = useState("");
    const [reviewPhotoComposition, setReviewPhotoComposition] = useState<"En la mano" | "Sobre una superficie">("En la mano");
    const [reviewPhotoWho, setReviewPhotoWho] = useState(UGC_WHO_OPTIONS[0]);
    const [reviewPhotoEthnicity, setReviewPhotoEthnicity] = useState(UGC_ETHNICITIES[0]);
    const [reviewPhotoGenerating, setReviewPhotoGenerating] = useState(false);
    const [reviewPhotoResults, setReviewPhotoResults] = useState<{ image: string }[]>([]);

    // Video UGC (Veo) — apariencia de la persona (Paso 0) + guion (Paso 1) + generación de video (Paso 2)
    const [veoGender, setVeoGender] = useState("Mujer");
    const [veoAge, setVeoAge] = useState("25-34");
    const [veoEthnicity, setVeoEthnicity] = useState(UGC_ETHNICITIES[0]);
    const [veoHairColor, setVeoHairColor] = useState(UGC_HAIR_COLORS[0]);
    const [veoHairStyle, setVeoHairStyle] = useState(UGC_HAIR_STYLES[0]);
    const [veoExtraDetails, setVeoExtraDetails] = useState("");
    ...
    const handleGenerateUgc = async () => {
        ...
        const mirrorInstruction = ugcPhotoType === 'Espejo' ? " Since the composition is a mirror selfie ('Espejo'), the model must be holding an iPhone in one hand, taking a mirror selfie photo of themselves in front of a mirror, with the phone visible and the product realistically visible in the shot/reflection." : "";

        const productName = ugcProduct || activeProject?.productName || "producto";
        const realismLevel = UGC_REALISM_LEVELS.find(r => r.id === ugcRealism) || UGC_REALISM_LEVELS[0];
        const extraInstructionsBlock = ugcExtraInstructions.trim() ? `\nINSTRUCCIONES ADICIONALES DEL USUARIO: ${ugcExtraInstructions.trim()}.` : "";

        const finalPrompt = `UGC PHOTO STYLE: Real photo taken by a customer or content creator. Authentic amateur look, social media style: ${ugcStyle}. Camera framing: ${ugcCameraAngle}. Lighting: ${ugcLighting}, time of day: ${ugcTimeOfDay}. Post-processing look: ${ugcFilter}.
SUBJECT: A highly attractive, photogenic ${ugcWho} model (${ugcAge} years old, ${ugcEthnicity} ethnicity, based in ${ugcCountry}, ${ugcBodyType} body type, ${ugcHairColor} ${ugcHairStyle} hair) looking directly at the camera, making eye contact, with a ${ugcExpression} facial expression. The model must look like a real, authentic person. Wardrobe/outfit style: ${ugcOutfit}, stylishly well-dressed and appropriate for the scenario.
REALISM: ${realismLevel.instruction}
SCENARIO: In a ${ugcScenario}.
PHOTO COMPOSITION: ${ugcPhotoType} showing the model holding the product "${productName}" naturally. The model is looking directly into the camera lens. The product "${productName}" must NOT float in the air and must NOT be duplicated or copied.${mirrorInstruction}
        ...
        try {
            ...
            if (part?.inlineData) {
                ...
                const newResult = {
                    ...
                    desc: `Modelo ${ugcWho} (${ugcAge} años, ${ugcEthnicity}) - Escenario: ${ugcScenario} - ${ugcCameraAngle}, ${ugcLighting} - Estilo: ${ugcStyle} (${ugcPhotoType})`
                };
...
CRITICAL RULE: The model must ALWAYS look directly at the camera. The product must NOT float in the air and must NOT be duplicated or copied. The final image must look like a real, high-quality, organic photo taken by an attractive creator, not a professional studio render. No text, logos, or artificial overlays. DIMENSIONS: 1:1.`;
    ...
    // --- Fotos de Reseña: foto de producto en mano/sobre superficie + texto de reseña ---
    const generateReviewPhotoImage = async (): Promise<string> => {
        const inputImg = reviewPhotoProductImage || activeProject?.productPreview;
        if (!inputImg) throw new Error("Sube una foto de tu producto primero.");

        const base64Data = inputImg.split(",")[1] || "";
        const mimeType = inputImg.includes("image/png") ? "image/png" : "image/jpeg";
        const productName = reviewPhotoProduct || activeProject?.productName || "producto";
        const holdInstruction = reviewPhotoComposition === "En la mano"
            ? `holding the product "${productName}" naturally in hand, close to the camera, in a casual POV-style close-up`
            : `with the product "${productName}" placed naturally on a table, counter, shelf, or floor (not floating, not held by anyone) in a casual everyday setting`;

        const prompt = `UGC PHOTO STYLE: Real, authentic photo taken by a genuine customer to accompany a written product review. Amateur, unpolished, iPhone/social-media quality — NOT a professional studio photo. Reference look: a close-up POV product shot like the ones customers post on Amazon/Instagram reviews — NOT a portrait or selfie of a person.
SUBJECT: A close-up, POV-style shot showing only a ${reviewPhotoWho.toLowerCase()}'s hand (${reviewPhotoEthnicity} skin tone) ${holdInstruction}. NO face, NO body, and NO other person may be visible anywhere in the frame — just the hand/fingers and the product itself, exactly like the reference product photos, never a portrait or a person looking at the camera.
BACKGROUND: An ordinary, everyday indoor setting (e.g. a wood table, bathroom counter, bedroom floor, closet, kitchen counter) softly out of focus, as if casually snapped on a phone — not a studio backdrop.
The product "${productName}" from the uploaded image must be integrated naturally and realistically into the scene, matching the real product exactly — it must NOT float in the air and must NOT be duplicated or copied.
CRITICAL RULE: The final image must look like a real photo a customer would actually attach to their own review — natural lighting, everyday background, no text, logos, or artificial overlays, and absolutely no visible face, body, or person. DIMENSIONS: 1:1.`;
        ...
        return withRetry(async () => {
            ...
            const res = await fetch(url, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ contents: [{ parts: [{ inlineData: { mimeType, data: base64Data } }, { text: prompt }] }] })
            });

            if (!res.ok) {
                const retryAfterMs = parseRetryAfterMs(res);
                let message = `Error Gemini (${res.status})`;
                try {
                    const errData = await res.json();
                    message = errData?.error?.message || message;
                } catch { /* respuesta no era JSON */ }
                throw new RetryableHttpError(res.status === 429 ? "Google Rate Limit (429). Espera unos minutos." : message, res.status, retryAfterMs);
            }

            const data = await res.json();
            if (data.error) throw new RetryableHttpError(data.error.message || "Error Gemini", 500);

            const part = data.candidates?.[0]?.content?.parts?.[0];
            if (!part?.inlineData) throw new RetryableHttpError("Gemini no devolvió una imagen.", 500);

            return `data:${part.inlineData.mimeType};base64,${part.inlineData.data}`;
        }, { maxRetries: 3, baseDelayMs: 1500 });
    };

    const handleGenerateReviewPhoto = async () => {
        if (!apiKey) {
            setToast({ msg: "⚠️ Configura tu API Key en la pestaña de Configuración para activar la IA", type: 'error' });
            return;
        }
        if (!(reviewPhotoProductImage || activeProject?.productPreview)) {
            setToast({ msg: "Sube una foto de tu producto en el panel izquierdo primero", type: 'error' });
            return;
        }

        setReviewPhotoGenerating(true);
        try {
            const image = await generateReviewPhotoImage();
            setReviewPhotoResults(prev => [{ image }, ...prev]);
            setToast({ msg: "✅ Foto de reseña generada con éxito", type: 'success' });
        } catch (error: any) {
            setToast({ msg: error?.message || "Error en la generación.", type: 'error' });
        } finally {
            setReviewPhotoGenerating(false);
        }
    };

    // --- Video UGC (Veo): Paso 1, guion generado con Gemini texto, client-side con la key propia del usuario ---
    // NOTA: a diferencia de otras generaciones de texto de la app, esta llamada NO usa /api/vertex-ai/generate-text
    // (esa ruta llama a OpenAI server-side con la key del entorno) — el usuario pidió explícitamente Gemini con su
    // propia key, así que se replica el mismo patrón client-side fetch que ya usan las imágenes.
    const generateVideoScript = async () => {
        ...
        try {
            const productName = activeProject.productName || activeProject.name || "el producto";
            const prompt = `Escribe un guion de VENTA DIRECTA en ESPAÑOL para un video vertical de 8 segundos, estilo selfie/handheld, como si un cliente real grabara con su celular mostrando el producto "${productName}" a cámara — el objetivo es VENDER, no solo mostrar el producto.

El diálogo debe seguir esta estructura de ritmo exacta, pensada para los 8 segundos completos:
- 0–2 seg: HOOK que detenga el scroll — una afirmación fuerte, una pregunta directa, o el problema que resuelve el producto, dicho de inmediato. Sin saludo, sin introducción.
- 2–5 seg: El BENEFICIO concreto o la transformación que da el producto, en una frase clara y específica (nada genérico).
- 5–7 seg: PRUEBA SOCIAL o URGENCIA — algo como "a mí me funcionó", un resultado tangible, o una razón para actuar ya (escasez, oferta, etc.).
- 7–8 seg: CTA directo y corto — qué debe hacer el espectador ahora mismo (ej. "dale al link", "cómpralo ya", "no lo pienses más").

REGLAS:
- El diálogo total debe poder decirse cómodamente en 8 segundos a ritmo normal: máximo 20-24 palabras en total, sin excepción.
- Nada de rodeos, saludos ("hola chicos", etc.) ni frases de relleno — directo al grano desde la primera palabra.
- El TONO sigue siendo auténtico y casero, como si lo dijera una persona real, no un comercial leído — pero el CONTENIDO debe ser 100% orientado a vender, no solo a mostrar o describir el producto.
- El diálogo debe fluir natural, sin marcar los tramos con etiquetas — la estructura es interna, no debe notarse como una fórmula.

Responde en este formato exacto:
TONO: (ej. entusiasta, casual, cercano)
ACCIÓN/GESTO: descripción breve de qué hace la persona con el producto, reforzando el mensaje de venta (ej. "sostiene el producto cerca de cámara y lo señala directo al decir el CTA")
LO QUE DICE: el diálogo exacto, siguiendo la estructura de ritmo de arriba, fluido y natural.
Responde SOLO con el guion en ese formato, sin explicaciones adicionales.`;

            const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`;
            const script: string = await withRetry(async () => {
                const res = await fetch(url, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                ...
                if (!text) throw new RetryableHttpError("Gemini no devolvió un guion.", 500);
                return String(text).trim();
            }, { maxRetries: 2, baseDelayMs: 1200 });

            setVideoScript(script);
            setToast({ msg: "✅ Guion generado con éxito", type: 'success' });
        } catch (error: any) {
            setToast({ msg: error?.message || "Error al generar el guion.", type: 'error' });
        } finally {
            setIsGeneratingScript(false);
        }
    };

    // --- Video UGC (Veo): Paso 2, generación del video con Veo (predictLongRunning + polling) ---
    const handleGenerateVeoVideo = async () => {
        ...
        if (!activeProject?.productPreview) {
            setToast({ msg: "Selecciona un proyecto con una foto de producto cargada", type: 'error' });
            return;
        }
        if (!videoScript.trim()) {
            setToast({ msg: "Genera o escribe un guion antes de continuar", type: 'error' });
            return;
        }

        const projectId = activeProject.id;
        const resultId = Math.random().toString(36).substr(2, 9);
        appendVideoResult(projectId, { id: resultId, script: videoScript, status: 'generando', createdAt: Date.now() });
        ...
        try {
            ...
            const genderText = veoGender === "Hombre" ? "a man" : "a woman";
            const extraDetailsText = veoExtraDetails.trim() ? ` Additional appearance details: ${veoExtraDetails.trim()}.` : "";

            const prompt = `FINAL OUTPUT ONLY (mandatory, most important instruction): Generate ONLY
the raw video footage itself, filling the entire frame edge-to-edge. This
is NOT a screen recording and there must be NO phone screen visible
anywhere in the shot. Do NOT show: a phone screen, phone bezel or frame,
status bar, clock/time, battery icon, wifi/signal icons, notification bar,
home indicator bar, camera app viewfinder, capture/shutter button, front/
back camera flip icon, filter or sticker icons, username/handle text
overlay, or any app UI/chrome of any kind. Just the person and the room,
exactly as a finished, already-recorded video looks when played back with
zero UI on top of it.
Selfie-style, handheld, authentic UGC video, 8 seconds, vertical (9:16).
A real person films themselves with their phone's front camera, holding the
product close to the camera in one hand.
PERSON: The person on camera is ${genderText}, around ${veoAge} years old,
${veoEthnicity} ethnicity, with ${veoHairColor.toLowerCase()} ${veoHairStyle.toLowerCase()}
hair.${extraDetailsText}
SCENE: Filmed in a bedroom or living room during the daytime, lit by soft,
moderate ambient indoor light — NOT bright or overexposed, and the person
is NOT standing directly in front of a window or backlit by one. The window
(if any) is out of frame or off to the side, contributing only gentle,
diffused fill light, with subtle natural shadow — not a dramatic bright/dark
split across the face. The background shows an ordinary lived-in room (a
bed, a wall, everyday clutter softly out of focus), lit at a normal indoor
brightness level, like a typical phone photo taken indoors, not near a
sunlit window.
COLOR & WHITE BALANCE (mandatory):
- Neutral daylight white balance — skin tones look natural and true-to-life,
  NOT warm, NOT orange, NOT sepia, NOT golden-hour toned.
- The overall color temperature should match an ordinary indoor phone photo
  taken during the day: whites look white, not yellow or amber.
- Soft, cool-neutral light from a window (like an overcast or midday sky),
  not warm tungsten/lamp light and not sunset/golden-hour light.
VISUAL STYLE: Shot on an iPhone front camera. Slightly grainy digital image,
mild motion blur from handheld movement, natural skin tones without
retouching, visible minor shake as the person adjusts the phone angle.
Colors look like unedited phone footage — not color-graded, not
professionally exposed.
FRAMING & EXPRESSION: Imperfect front-camera framing, like a real person
holding their phone up to talk — slightly off-center, a bit tilted, camera
at arm's length, sometimes cropping the top of the head or the edge of
the frame. Candid, mid-conversation facial expression: natural blinking,
mouth actually moving through the words as they talk, small genuine
imperfections — NOT a static posed smile held the whole clip. Casual,
everyday clothing (hoodie, tank top, plain top) — not styled, not
influencer-polished. The overall feel should read like an actual TikTok/
Instagram Reels clip a real customer filmed and posted, not a produced ad.
SKIN & TEXTURE REALISM (mandatory): Real, visible skin texture — natural
pores, subtle imperfections, slight uneven tone — NOT smooth, airbrushed,
or filtered skin. Avoid any "beauty filter" gloss or plastic-looking skin.
Facial features have natural, slightly asymmetric human imperfection —
avoid an overly symmetric, "too perfect" AI-generated look. Natural eye
movement and blinking, with real catchlights — avoid a glassy, vacant, or
overly smooth stare. Overall image quality should resemble a real video
uploaded to Instagram/TikTok: slight compression artifacts, not a clean
rendered/polished image.
REMINDER: no phone screen, screen bezel, status bar, or any UI/app chrome
anywhere in the frame — only the raw recorded footage of the person and
the room.
Base the spoken dialogue and action on this script: ${videoScript}`;

            const startUrl = `https://generativelanguage.googleapis.com/v1beta/models/${VEO_MODEL}:predictLongRunning?key=${apiKey}`;
            const operationName: string = await withRetry(async () => {
                const res = await fetch(startUrl, {
                    ...
                    body: JSON.stringify({
                        instances: [{
                            prompt,
                            image: { bytesBase64Encoded: base64Data, mimeType }
                        }],
                        parameters: {
                            aspectRatio: "9:16",
                            durationSeconds: VEO_CLIP_DURATION_SECONDS,
                ...
                if (!res.ok) {
                    const retryAfterMs = parseRetryAfterMs(res);
                    let message = `Error Veo (${res.status})`;
                    try {
                        const errData = await res.json();
                        message = errData?.error?.message || message;
                    } catch { /* respuesta no era JSON */ }
                ...
                const data = await res.json();
                if (!data.name) throw new RetryableHttpError("Veo no devolvió una operación válida.", 500);
                return data.name as string;
            }, { maxRetries: 2, baseDelayMs: 2000 });

            // Polling: cada 10s, hasta ~60 intentos (~10 min) antes de marcar timeout como error.
            // "Todavía generando" NO es un error — solo se reintenta (withRetry) un fallo de red/5xx del propio poll.
            let videoUri: string | null = null;
            let attempts = 0;
            const maxAttempts = 60;
            while (attempts < maxAttempts && !videoUri) {
                await new Promise(resolve => setTimeout(resolve, 10000));
                attempts++;

                const pollResult: any = await withRetry(async () => {
                    const pollUrl = `https://generativelanguage.googleapis.com/v1beta/${operationName}?key=${apiKey}`;
                    const res = await fetch(pollUrl);
                    if (!res.ok) {
                        const retryAfterMs = parseRetryAfterMs(res);
                        throw new RetryableHttpError(`Error consultando el estado del video (${res.status})`, res.status, retryAfterMs);
                    }
                    return res.json();
                }, { maxRetries: 2, baseDelayMs: 2000 });

                if (pollResult.error) {
                    throw new Error(pollResult.error.message || "Veo falló al generar el video.");
                }
                if (pollResult.done) {
                    const sample = pollResult.response?.generateVideoResponse?.generatedSamples?.[0];
                    if (!sample?.video?.uri) {
                        throw new Error("Veo terminó pero no devolvió un video (posible rechazo de contenido por las políticas de Google).");
                    }
            ...
            if (!videoUri) {
                throw new Error("Tiempo de espera agotado generando el video. Intenta de nuevo.");
            }

            // El video NO se sube a ningún storage externo (decisión del usuario: sin Supabase) — se
            // reproduce directamente desde Google a través de un pequeño proxy server-side propio.
            // Es necesario porque un <video>/<a> del navegador no puede mandar el header x-goog-api-key
            // que Google exige para descargar el video; el proxy solo reenvía los bytes, no persiste nada.
            // Por lo tanto el video deja de reproducirse quedando el enlace de Google expirado (~48h).
            const videoUrl = `/api/veo/video-proxy?uri=${encodeURIComponent(videoUri)}&key=${encodeURIComponent(apiKey)}`;

            updateVideoResult(projectId, resultId, { status: 'listo', videoUrl });
            setToast({ msg: "✅ Video generado con éxito", type: 'success' });
        } catch (error: any) {
            updateVideoResult(projectId, resultId, { status: 'error', errorMessage: error?.message || "Error desconocido" });
            setToast({ msg: error?.message || "Error al generar el video.", type: 'error' });
        } finally {
    ...
    const downloadWaCapture = async (format: 'png' | 'jpg') => {
        ...
        try {
            ...
            if (format === 'png') {
                ...
                dataUrl = await toJpeg(node, { cacheBust: true, pixelRatio: 2, quality: 0.95 });
            }
...
<body>`;
    ...
    const [batchCount, setBatchCount] = useState<typeof BATCH_COUNT_OPTIONS[number]>(1);
    ...
    // Append seguro ante concurrencia: usa la forma funcional de setState (a diferencia
    // de updateActiveProject, que lee `projects` del closure) para que varios creativos
    // de un mismo lote terminando casi al mismo tiempo no se pisen entre sí.
    const appendProjectResult = (projectId: string, newResult: Project["results"][number]) => {
        setProjects(prev => prev.map(p => {
            if (p.id !== projectId) return p;
            return { ...p, results: [...(p.results || []), newResult], updatedAt: Date.now() };
        }));
    };

    // Mismo espíritu que appendProjectResult/updateActiveProject pero para Project.videoResults —
    // funcional (concurrency-safe) porque el polling de Veo actualiza el mismo item repetidas veces
    // mientras el usuario puede seguir interactuando con la app.
    const appendVideoResult = (projectId: string, newResult: NonNullable<Project["videoResults"]>[number]) => {
        setProjects(prev => prev.map(p => {
            if (p.id !== projectId) return p;
            return { ...p, videoResults: [newResult, ...(p.videoResults || [])], updatedAt: Date.now() };
        }));
    };

    const updateVideoResult = (projectId: string, resultId: string, updates: Partial<NonNullable<Project["videoResults"]>[number]>) => {
        setProjects(prev => prev.map(p => {
            if (p.id !== projectId) return p;
            return {
                ...p,
                videoResults: (p.videoResults || []).map(r => r.id === resultId ? { ...r, ...updates } : r),
                updatedAt: Date.now()
            };
        }));
    };
    ...
    const handleGenerate = async (angleId?: string, count: number = 1) => {
        ...
        setGenProgress({ done: 0, total: count, failed: 0 });
        ...
        const goalCtx = { outputLang, productName: activeProject.productName, targetAudience: activeProject.targetAudience };

        // count === 1: comportamiento idéntico al histórico (solo la plantilla elegida
        // manualmente, sin gancho forzado). count > 1: cada creativo del lote recibe su
        // propia combinación única de plantilla visual + gancho de copy.
        const singleAdType = AD_TYPES.find(a => a.id === (angleId || "HERO")) || AD_TYPES.find(a => a.id === "HERO")!;
        const combos: CreativeCombo[] = count === 1
            ? [{ adType: singleAdType, hook: null }]
            : buildBatchCombos(count);
        ...
        const generateOneCreative = async (combo: CreativeCombo, index: number) => {
            const { adType, hook } = combo;
            try {
                await withRetry(async () => {
                    ...
                    const hookInstruction = hook ? ` GANCHO/HOOK OBLIGATORIO DEL MENSAJE (debe notarse en el titular/copy): ${hook.instruction}` : "";
                    const finalPrompt = `Create a high quality commercial ad image. Professional photography style: ${adType.style}. AD CREATIVE OBJECTIVE: ${adType.goal(goalCtx)}.${hookInstruction} ${brandingContext} ${customContext} ${priceOverlay} ${ctaOverlay} DIMENSIONS: ${selectedOutputSize || '1:1'}. Variation ${index + 1}`;
                    ...
                    const part = data.candidates?.[0]?.content?.parts?.[0];
                    if (!part?.inlineData) throw new RetryableHttpError("Gemini no devolvió una imagen para este creativo.", 500);

                    const angleLabel = hook ? `${adType.name} · ${hook.label}` : adType.name;
                    const newVar = { image: `data:${part.inlineData.mimeType};base64,${part.inlineData.data}`, title: adType.name, copy: "", angle: angleLabel };

                    // Genera el copy con la MISMA ruta existente; lo único nuevo es que el
                    // string de "angle" que le mandamos ahora incluye el gancho asignado,
                    // para que el copy varíe junto con la imagen.
                    try {
                        const copyRes = await fetch('/api/vertex-ai/generate-copy', {
                            ...
                            body: JSON.stringify({
                                ...
                                angle: hook ? `${adType.name} — Gancho: ${hook.label}. ${hook.instruction}` : adType.name,
                    ...
                    appendProjectResult(projectId, newVar);
                }, {
                    maxRetries: 3,
                    baseDelayMs: 1500,
                    onRetry: (attempt, err: any) => {
                        console.warn(`Reintento ${attempt} para creativo ${index + 1}/${count}:`, err?.message || err);
                    }
                });
            } catch (err: any) {
                failedCount += 1;
                lastErrorMessage = err?.message || "Error desconocido en la generación.";
                console.error(`Creativo ${index + 1}/${count} falló tras reintentos:`, err);
            } finally {
                setGenProgress(prev => prev ? { ...prev, done: prev.done + 1, failed: failedCount } : prev);
            }
        };

        try {
            await runWithConcurrency(combos, generateOneCreative, count === 1 ? 1 : BATCH_CONCURRENCY);

            if (failedCount === 0) {
                setToast({ msg: count === 1 ? "¡Creativo generado!" : `¡${count}/${count} creativos generados!`, type: 'success' });
            } else if (failedCount < count) {
                setToast({ msg: `Se generaron ${count - failedCount}/${count} creativos (${failedCount} fallaron tras reintentos).`, type: 'error' });
            } else {
                setToast({ msg: lastErrorMessage || "No se pudo generar ningún creativo. Revisa tu API Key o intenta de nuevo.", type: 'error' });
            }
        } catch (error: any) {
            setToast({ msg: error.message || "Error en la generación.", type: 'error' });
        } finally {
        ...
        <div style={{ background: "#030303", minHeight: "100vh", color: "#fff", fontFamily: "Inter, sans-serif", display: "flex" }}>
            ...
            <aside style={{ width: 280, borderRight: "1px solid rgba(255,255,255,0.05)", padding: "40px 24px", display: "flex", flexDirection: "column", height: "100vh", position: "fixed", background: "#050505" }}>
                ...
                <nav style={{ display: "flex", flexDirection: "column", gap: 6, flex: 1, overflowY: 'auto' }} className="custom-scrollbar">
                    ...
                    <div className={`nav-item ${activeTab === 'video_ugc' ? 'active' : ''}`} onClick={() => { setActiveTab('video_ugc'); setActiveProjectId(null); }}><Video size={18} /> Video UGC (Veo) <span style={{ fontSize: 9, background: "#EC4899", color: "#fff", padding: "2px 6px", borderRadius: 4, marginLeft: 8, fontWeight: 900 }}>NUEVO</span></div>
                    ...
                    <div className={`nav-item ${activeTab === 'review_photos' ? 'active' : ''}`} onClick={() => { setActiveTab('review_photos'); setActiveProjectId(null); }}><Award size={18} /> Fotos de Reseña <span style={{ fontSize: 9, background: "#F59E0B", color: "#fff", padding: "2px 6px", borderRadius: 4, marginLeft: 8, fontWeight: 900 }}>NUEVO</span></div>
            ...
            <main style={{ marginLeft: 300, flex: 1, padding: "40px 20px", overflowX: 'hidden' }}>
                    ...
                    <div style={{ width: "100%" }} className="anim-in">
                        ...
                        <div style={{ marginTop: 32 }} className="glass-card">
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 24, padding: 32 }}>
                                ...
                                <div className="home-tool-card" onClick={() => setActiveTab('review_photos')}>
                                    ...
                                    <div>
                                        <div style={{ fontWeight: 800 }}>Fotos de Reseña</div>
                                        <div style={{ fontSize: 11, color: "#6B7280" }}>Foto + texto de reseña</div>
                                    </div>
                                </div>
                                <div className="home-tool-card" onClick={() => setActiveTab('video_ugc')}>
                                    <div style={{ width: 44, height: 44, borderRadius: 12, background: "rgba(236, 72, 153, 0.1)", display: "flex", alignItems: "center", justifyContent: "center", color: "#EC4899" }}>
                                        <Video size={24} />
                                    </div>
                                    <div>
                                        <div style={{ fontWeight: 800 }}>Video UGC (Veo)</div>
                                        <div style={{ fontSize: 11, color: "#6B7280" }}>Video selfie con producto en mano</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: "400px 1fr", gap: 40 }}>
                                    <div className="glass-card" style={{ padding: 40, height: "fit-content" }}>
                                        <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
                                            ...
                                            <div>
                                                <label style={{ display: "block", fontSize: 12, fontWeight: 800, color: activeProject?.primaryColor, marginBottom: 12 }}>CANTIDAD DE CREATIVOS</label>
                                                    ...
                                                    {BATCH_COUNT_OPTIONS.map(n => (
                                                        <button
                                                            ...
                                                            style={{
                                                                ...
                                                                border: batchCount === n ? `2px solid ${getIconColor(activeProject?.primaryColor)}` : "1px solid rgba(255,255,255,0.1)",
                                                                background: batchCount === n ? `${getIconColor(activeProject?.primaryColor)}20` : "transparent",
                                                                color: batchCount === n ? getIconColor(activeProject?.primaryColor) : "#9CA3AF"
                                                            }}
                                                        >
                                                            {n}
                                                        </button>
                                                ...
                                                {batchCount > 1 && (
                                                    <p style={{ fontSize: 11, color: "#9CA3AF", marginBottom: 12 }}>
                                                        Modo lote: cada creativo combina automáticamente una plantilla visual distinta con un gancho de copy distinto (dolor, beneficio, urgencia, prueba social, curiosidad, escasez), no solo variaciones cosméticas.
                                                    </p>
                                                )}
                                            </div>

                                            <div style={{ opacity: batchCount > 1 ? 0.4 : 1, pointerEvents: batchCount > 1 ? "none" : "auto", transition: "opacity 0.2s" }}>
                                                <label style={{ display: "block", fontSize: 12, fontWeight: 800, color: activeProject?.primaryColor, marginBottom: 12 }}>ÁNGULO DE VENTA {batchCount > 1 ? "(se ignora en modo lote)" : ""}</label>
                                            ...
                                            <button className="btn-primary" style={{ width: "100%", justifyContent: "center", background: activeProject?.primaryColor }} onClick={() => handleGenerate(selectedAngle, batchCount)} disabled={isLoading}>{isLoading ? <Loader2 className="animate-spin" /> : <><Sparkles /> Generar Creativos</>}</button>
                                            {isLoading && genProgress && (
                                                <div style={{ marginTop: 12 }}>
                                                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#9CA3AF", marginBottom: 6 }}>
                                                        <span>Generando {genProgress.done}/{genProgress.total}{genProgress.failed > 0 ? ` · ${genProgress.failed} fallido${genProgress.failed === 1 ? "" : "s"} (reintentando)` : ""}</span>
                                                    ...
                                                    <div style={{ width: "100%", height: 6, borderRadius: 100, background: "rgba(255,255,255,0.08)", overflow: "hidden" }}>
                                                        <div style={{ width: `${(genProgress.done / genProgress.total) * 100}%`, height: "100%", background: activeProject?.primaryColor || "#8B5CF6", transition: "width 0.2s" }} />
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "400px 1fr", gap: 40, alignItems: "start", marginBottom: 40 }}>
                                    ...
                                    <div className="glass-card" style={{ padding: 32, height: "fit-content" }}>
                                        ...
                                        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                                            ...
                                            {/* Parámetros agrupados en secciones colapsables */}
                                            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>

                                                <UgcSection title="Modelo" isOpen={ugcOpenSections.modelo} onToggle={() => setUgcOpenSections(prev => ({ ...prev, modelo: !prev.modelo }))}>
                                                    <div>
                                                        ...
                                                        <select value={ugcWho} onChange={(e) => setUgcWho(e.target.value)} className="input-field">
                                                            {UGC_WHO_OPTIONS.map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Edad</label>
                                                        <select value={ugcAge} onChange={(e) => setUgcAge(e.target.value)} className="input-field">
                                                            {["18-24", "25-34", "35-44", "45-60"].map(v => <option key={v} value={v}>{v} años</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Etnia</label>
                                                        <select value={ugcEthnicity} onChange={(e) => setUgcEthnicity(e.target.value)} className="input-field">
                                                            {UGC_ETHNICITIES.map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>País / Mercado</label>
                                                        <select value={ugcCountry} onChange={(e) => setUgcCountry(e.target.value)} className="input-field">
                                                            {UGC_COUNTRIES.map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Tipo de cuerpo</label>
                                                        <select value={ugcBodyType} onChange={(e) => setUgcBodyType(e.target.value)} className="input-field">
                                                            {UGC_BODY_TYPES.map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                                                        <div>
                                                            <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Color de cabello</label>
                                                            <select value={ugcHairColor} onChange={(e) => setUgcHairColor(e.target.value)} className="input-field">
                                                                {UGC_HAIR_COLORS.map(v => <option key={v} value={v}>{v}</option>)}
                                                            </select>
                                                        </div>
                                                        <div>
                                                            <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Estilo de cabello</label>
                                                            <select value={ugcHairStyle} onChange={(e) => setUgcHairStyle(e.target.value)} className="input-field">
                                                                {UGC_HAIR_STYLES.map(v => <option key={v} value={v}>{v}</option>)}
                                                            </select>
                                                        </div>
                                                    </div>
                                                </UgcSection>

                                                <UgcSection title="Escena" isOpen={ugcOpenSections.escena} onToggle={() => setUgcOpenSections(prev => ({ ...prev, escena: !prev.escena }))}>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Escenario</label>
                                                        <select value={ugcScenario} onChange={(e) => setUgcScenario(e.target.value)} className="input-field">
                                                            {["Baño", "Cocina", "Gimnasio", "Playa", "Oficina", "Casa", "Auto", "Cafetería"].map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Hora del día</label>
                                                        <select value={ugcTimeOfDay} onChange={(e) => setUgcTimeOfDay(e.target.value)} className="input-field">
                                                            {UGC_TIME_OF_DAY.map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Vestuario</label>
                                                        <select value={ugcOutfit} onChange={(e) => setUgcOutfit(e.target.value)} className="input-field">
                                                            {UGC_OUTFITS.map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                </UgcSection>

                                                <UgcSection title="Fotografía" isOpen={ugcOpenSections.fotografia} onToggle={() => setUgcOpenSections(prev => ({ ...prev, fotografia: !prev.fotografia }))}>
                                                    <div>
                                                        ...
                                                        <select value={ugcPhotoType} onChange={(e) => setUgcPhotoType(e.target.value)} className="input-field">
                                                    ...
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Encuadre / Ángulo</label>
                                                        <select value={ugcCameraAngle} onChange={(e) => setUgcCameraAngle(e.target.value)} className="input-field">
                                                            {UGC_CAMERA_ANGLES.map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Estilo</label>
                                                        <select value={ugcStyle} onChange={(e) => setUgcStyle(e.target.value)} className="input-field">
                                                            {["iPhone", "Profesional", "TikTok", "Instagram", "Pinterest"].map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Iluminación</label>
                                                        <select value={ugcLighting} onChange={(e) => setUgcLighting(e.target.value)} className="input-field">
                                                            {UGC_LIGHTING.map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Filtro / Grano</label>
                                                        <select value={ugcFilter} onChange={(e) => setUgcFilter(e.target.value)} className="input-field">
                                                            {UGC_FILTERS.map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Expresión</label>
                                                        <select value={ugcExpression} onChange={(e) => setUgcExpression(e.target.value)} className="input-field">
                                                            {["Feliz", "Sorprendido", "Relajado", "Emocionado"].map(v => <option key={v} value={v}>{v}</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Nivel de realismo</label>
                                                        <select value={ugcRealism} onChange={(e) => setUgcRealism(e.target.value)} className="input-field">
                                                            {UGC_REALISM_LEVELS.map(r => <option key={r.id} value={r.id}>{r.label}</option>)}
                                                        </select>
                                                    </div>
                                                    <div>
                                                        ...
                                                        <select value={ugcOutputSize} onChange={(e) => setUgcOutputSize(e.target.value)} className="input-field">
                                                            {OUTPUT_SIZES.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
                                                        </select>
                                                    </div>
                                                </UgcSection>

                                                <UgcSection title="Extra" isOpen={ugcOpenSections.extra} onToggle={() => setUgcOpenSections(prev => ({ ...prev, extra: !prev.extra }))}>
                                                    <div>
                                                        <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Instrucciones adicionales</label>
                                                        <textarea
                                                            className="input-field"
                                                            placeholder="Ej: que use gafas de sol, que el fondo tenga plantas, que sostenga el producto con las dos manos..."
                                                            value={ugcExtraInstructions}
                                                            onChange={(e) => setUgcExtraInstructions(e.target.value)}
                                                            style={{ height: 70, fontSize: 12 }}
                                                        />
                                                    </div>
                                                </UgcSection>
                ...
                {activeTab === 'review_photos' && (
                    <div style={{ width: "100%" }}>
                        {!activeProjectId ? (
                            <>
                                <h1 style={{ fontSize: 32, fontWeight: 900, marginBottom: 40 }}>Mis Proyectos (Reseñas)</h1>
                                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 24 }}>
                                    <div onClick={() => setShowProjectModal(true)} className="project-card" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, border: "2px dashed rgba(245,158,11,0.3)" }}>
                                        <Plus size={24} /> <span style={{ fontWeight: 800 }}>Nuevo Proyecto</span>
                                    </div>
                                    {projects.filter(p => !p.type || p.type === 'ecommerce').map(project => (
                                        <div key={project.id} onClick={() => setActiveProjectId(project.id)} className="project-card">
                            ...
                            <div className="anim-in">
                                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 32 }}>
                                    <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
                                        <button onClick={() => setActiveProjectId(null)} style={{ color: "#9CA3AF", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontWeight: 700 }}><ArrowLeft size={16} /> Volver</button>
                                        <button onClick={() => { setReviewPhotoProductImage(null); setReviewPhotoResults([]); }} style={{ background: "rgba(239, 68, 68, 0.1)", border: "1px solid rgba(239, 68, 68, 0.2)", color: "#EF4444", padding: "6px 16px", borderRadius: 100, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>Empezar de nuevo</button>
                                    </div>
                                    <div style={{ textAlign: "right" }}>
                                        <div style={{ fontSize: 10, fontWeight: 700, opacity: 0.5 }}>PROYECTO ACTIVO</div>
                                        <div style={{ fontWeight: 900, fontSize: 24, fontFamily: activeProject?.font, color: activeProject?.primaryColor }}>{activeProject?.name}</div>
                                ...
                                <div style={{ marginBottom: 32 }}>
                                    <h1 style={{ fontSize: 32, fontWeight: 900, marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}>
                                        <Award size={28} color="#F59E0B" /> Fotos de Reseña
                                    </h1>
                                    <p style={{ color: "#9CA3AF" }}>Genera una foto del producto en la mano del cliente (o sobre una superficie) junto con el texto de la reseña, lista para usar como prueba social.</p>
                                </div>
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "400px 1fr", gap: 40, alignItems: "start", marginBottom: 40 }}>
                                    ...
                                    <div className="glass-card" style={{ padding: 32, height: "fit-content" }}>
                                        <h2 style={{ fontSize: 18, fontWeight: 800, marginBottom: 24, borderBottom: "1px solid rgba(255,255,255,0.07)", paddingBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                                            <Award size={18} color="#F59E0B" /> Reseña con Foto
                                        </h2>

                                        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>

                                            <div>
                                                <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#F59E0B", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 12 }}>
                                                    📸 Sube tu producto
                                                </label>
                                                <div style={{ border: "2px dashed rgba(255,255,255,0.1)", borderRadius: 24, height: 180, position: "relative", overflow: "hidden", background: "rgba(255,255,255,0.01)" }}>
                                                    {!(reviewPhotoProductImage || activeProject?.productPreview) ? (
                                                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 12, opacity: 0.3, padding: 20, textAlign: "center" }}>
                                                            <UploadCloud size={24} />
                                                            <span style={{ fontSize: 12 }}>Sube una foto de tu producto</span>
                                                            <input
                                                                ...
                                                                onChange={(e) => {
                                                                    ...
                                                                    if (file) {
                                                                        const reader = new FileReader();
                                                                        reader.onloadend = () => setReviewPhotoProductImage(reader.result as string);
                                                                        reader.readAsDataURL(file);
                                                                    }
                                                                }}
                                                                style={{ position: "absolute", inset: 0, opacity: 0, cursor: "pointer", zIndex: 10 }}
                                                        ...
                                                        <>
                                                            <img src={reviewPhotoProductImage || activeProject?.productPreview} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                                                            <button
                                                                onClick={() => {
                                                                    setReviewPhotoProductImage(null);
                                                                    if (activeProject) updateActiveProject({ productPreview: "" });
                                                                }}
                                                                style={{ position: "absolute", top: 12, right: 12, background: "rgba(0,0,0,0.6)", border: "none", color: "#fff", width: 28, height: 28, borderRadius: "50%", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 11 }}
                                                            >
                                                                <X size={14} />
                                            ...
                                            <div>
                                                <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#F59E0B", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>
                                                    Nombre del Producto
                                                </label>
                                                <input
                                                    className="input-field"
                                                    placeholder="Ej: Suero Capilar, Protector Solar..."
                                                    value={reviewPhotoProduct || activeProject?.productName || ""}
                                                    onChange={(e) => setReviewPhotoProduct(e.target.value)}
                                                />
                                            </div>
                                            ...
                                            <div>
                                                <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Composición</label>
                                                <select value={reviewPhotoComposition} onChange={(e) => setReviewPhotoComposition(e.target.value as "En la mano" | "Sobre una superficie")} className="input-field">
                                                    <option value="En la mano">En la mano</option>
                                                    <option value="Sobre una superficie">Sobre una superficie</option>
                                                </select>
                                            </div>

                                            <div>
                                                <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Quién sostiene el producto</label>
                                                <select value={reviewPhotoWho} onChange={(e) => setReviewPhotoWho(e.target.value)} className="input-field">
                                                    {UGC_WHO_OPTIONS.map(v => <option key={v} value={v}>{v}</option>)}
                                                </select>
                                            </div>
                                            ...
                                            <div>
                                                <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Etnia</label>
                                                <select value={reviewPhotoEthnicity} onChange={(e) => setReviewPhotoEthnicity(e.target.value)} className="input-field">
                                                    {UGC_ETHNICITIES.map(v => <option key={v} value={v}>{v}</option>)}
                                                </select>
                                            </div>
                                            ...
                                            <button
                                                type="button"
                                                onClick={handleGenerateReviewPhoto}
                                                disabled={reviewPhotoGenerating}
                                                style={{
                                                    width: "100%",
                                                    padding: "16px 24px",
                                                    borderRadius: 12,
                                            ...
                                            >
                                                {reviewPhotoGenerating ? <Loader2 className="animate-spin" size={18} /> : <><Award size={16} /> Generar Foto de Reseña</>}
                                            </button>
                                    ...
                                    <div>
                                        {reviewPhotoResults.length > 0 && (
                                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                                                <h3 style={{ fontSize: 20, fontWeight: 900 }}>Resultados Generados</h3>
                                                <button
                                                    onClick={() => downloadAsZip(reviewPhotoResults.map((r, idx) => ({ image: r.image, name: `review_${idx}` })), "clickads_reviews")}
                                                    className="btn-primary"
                                                    style={{ padding: "8px 16px", fontSize: 11, background: "rgba(255,255,255,0.05)", border: "1px solid #F59E0B", color: "#F59E0B" }}
                                                >
                                                    <Layers size={14} /> Descargar Todas (.ZIP)
                                                </button>
                                            </div>
                                        )}
                                        ...
                                        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 24 }}>
                                            ...
                                            {reviewPhotoGenerating && (
                                                <div className="glass-card" style={{ padding: 40, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, minHeight: 320, gridColumn: "1 / -1", border: "1px dashed rgba(245, 158, 11, 0.4)" }}>
                                                    <Loader2 className="animate-spin" size={40} style={{ color: "#F59E0B" }} />
                                                    <span style={{ fontSize: 14, color: "#FBBF24", fontWeight: 700, letterSpacing: "0.05em", textAlign: "center" }}>GENERANDO FOTO CON IA...</span>
                                                    <span style={{ fontSize: 11, color: "#6B7280" }}>Por favor espera unos segundos.</span>
                                                </div>
                                            )}

                                            {reviewPhotoResults.map((res, i) => (
                                                <div key={i} className="glass-card" style={{ padding: 16, position: "relative", display: "flex", flexDirection: "column" }}>
                                                    <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", marginBottom: 12 }}>
                                                        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                                                            <button onClick={() => downloadSingleImage(res.image, `review_${i}.jpg`)} style={{ background: "none", border: "none", color: "#F59E0B", cursor: "pointer" }}><Download size={16} /></button>
                                                            <button onClick={() => saveToLibrary(res.image, "Reseña con Foto")} style={{ background: "none", border: "none", color: "#F59E0B", cursor: "pointer" }}>
                                                                <Bookmark size={16} fill={library.some(a => a.image === res.image) ? "#F59E0B" : "none"} />
                                                            </button>
                                                            <button onClick={() => setZoomedImage(res.image)} style={{ background: "none", border: "none", color: "#F59E0B", cursor: "pointer" }}>
                                                                <Search size={16} />
                                                            </button>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div
                                                        style={{ position: "relative", cursor: "zoom-in", overflow: "hidden", borderRadius: 16, aspectRatio: "1/1", background: "#0d0d14" }}
                                                        onClick={() => setZoomedImage(res.image)}
                                                        className="image-zoom-container"
                                                    >
                                                        {res.image ? <img src={res.image} style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 0.3s ease" }} className="hover-zoom" /> : (
                                                            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "#6B7280", fontSize: 12, textAlign: "center", padding: 20 }}>La foto falló, intenta de nuevo.</div>
                                                        )}
                                                    </div>
                                                </div>
                                            ))}

                                            {reviewPhotoResults.length === 0 && !reviewPhotoGenerating && (
                                                <div style={{ gridColumn: "1 / -1", textAlign: "center", padding: "80px 20px" }}>
                                                    <Award size={40} style={{ color: "#F59E0B", opacity: 0.25, marginBottom: 16, margin: "0 auto" }} />
                                                    <h3 style={{ fontSize: 16, fontWeight: 700, color: "#E5E7EB", marginBottom: 8 }}>No hay fotos generadas aún</h3>
                                                    <p style={{ color: "#6B7280", fontSize: 13, maxWidth: 420, margin: "0 auto" }}>Sube la foto de tu producto en el panel izquierdo y haz clic en "Generar Foto de Reseña".</p>
                                                </div>
                                            )}
                ...
                {activeTab === 'video_ugc' && (
                    <div style={{ width: "100%" }}>
                        {!activeProjectId ? (
                            <>
                                <h1 style={{ fontSize: 32, fontWeight: 900, marginBottom: 40 }}>Mis Proyectos (Video UGC)</h1>
                                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 24 }}>
                                    <div onClick={() => setShowProjectModal(true)} className="project-card" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, border: "2px dashed rgba(236,72,153,0.3)" }}>
                                        <Plus size={24} /> <span style={{ fontWeight: 800 }}>Nuevo Proyecto</span>
                                    </div>
                                    {projects.filter(p => !p.type || p.type === 'ecommerce').map(project => (
                                        <div key={project.id} onClick={() => setActiveProjectId(project.id)} className="project-card">
                                            ...
                                            <h3 style={{ fontSize: 18, fontWeight: 800, fontFamily: project.font }}>{project.name}</h3>
                                            <p style={{ fontSize: 12, color: "#9CA3AF", marginTop: 4 }}>{(project.videoResults || []).length} videos</p>
                                        </div>
                                    ))}
                                </div>
                            </>
                            ...
                            <div className="anim-in">
                                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 32 }}>
                                    <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
                                        <button onClick={() => setActiveProjectId(null)} style={{ color: "#9CA3AF", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontWeight: 700 }}><ArrowLeft size={16} /> Volver</button>
                                        <button onClick={() => { setVideoScript(""); setScriptAccepted(false); }} style={{ background: "rgba(239, 68, 68, 0.1)", border: "1px solid rgba(239, 68, 68, 0.2)", color: "#EF4444", padding: "6px 16px", borderRadius: 100, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>Empezar de nuevo</button>
                                    </div>
                                    <div style={{ textAlign: "right" }}>
                                        <div style={{ fontSize: 10, fontWeight: 700, opacity: 0.5 }}>PROYECTO ACTIVO</div>
                                        <div style={{ fontWeight: 900, fontSize: 24, fontFamily: activeProject?.font, color: activeProject?.primaryColor }}>{activeProject?.name}</div>
                                ...
                                <div style={{ marginBottom: 32 }}>
                                    <h1 style={{ fontSize: 32, fontWeight: 900, marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}>
                                        <Video size={28} color="#EC4899" /> Video UGC (Veo)
                                    </h1>
                                    <p style={{ color: "#9CA3AF" }}>Genera un video selfie/handheld de 8 segundos con tu producto en mano, estilo casero y auténtico, usando Veo de Google.</p>
                                </div>

                                {!activeProject?.productPreview && (
                                    <div className="glass-card" style={{ padding: 24, marginBottom: 24, border: "1px solid rgba(239,68,68,0.3)", color: "#F87171", fontSize: 13 }}>
                                        ⚠️ Este proyecto no tiene una foto de producto cargada. Súbela en la pestaña "Genera Creativos" antes de continuar — Veo la necesita para anclar el video al producto real.
                                    </div>
                                )}
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "400px 1fr", gap: 40, alignItems: "start", marginBottom: 40 }}>
                                    ...
                                    <div className="glass-card" style={{ padding: 32, height: "fit-content" }}>
                                        ...
                                        {/* Producto */}
                                        <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 24, paddingBottom: 20, borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                                            {activeProject?.productPreview ? (
                                                <img src={activeProject.productPreview} style={{ width: 48, height: 48, borderRadius: 10, objectFit: "cover" }} />
                                            ) : (
                                                <div style={{ width: 48, height: 48, borderRadius: 10, background: "rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                                    <Box size={20} style={{ opacity: 0.4 }} />
                                                </div>
                                            )}
                                            <div>
                                                <div style={{ fontSize: 10, fontWeight: 800, color: "#EC4899", textTransform: "uppercase", letterSpacing: "0.05em" }}>Producto</div>
                                                <div style={{ fontWeight: 800, fontSize: 14 }}>{activeProject?.productName || activeProject?.name || "Sin nombre"}</div>
                                            </div>
                                        </div>

                                        {/* Apariencia de la persona */}
                                        <h2 style={{ fontSize: 15, fontWeight: 800, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                                            <User size={16} color="#EC4899" /> Apariencia de la persona
                                        </h2>
                                        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 24, paddingBottom: 20, borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                                                <div>
                                                    <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>¿Quién aparece?</label>
                                                    <select value={veoGender} onChange={(e) => setVeoGender(e.target.value)} className="input-field">
                                                        {["Mujer", "Hombre"].map(v => <option key={v} value={v}>{v}</option>)}
                                                    </select>
                                                </div>
                                                <div>
                                                    <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Edad</label>
                                                    <select value={veoAge} onChange={(e) => setVeoAge(e.target.value)} className="input-field">
                                                        {["18-24", "25-34", "35-44", "45-60"].map(v => <option key={v} value={v}>{v} años</option>)}
                                                    </select>
                                                </div>
                                            </div>
                                            <div>
                                                <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Etnia</label>
                                                <select value={veoEthnicity} onChange={(e) => setVeoEthnicity(e.target.value)} className="input-field">
                                                    {UGC_ETHNICITIES.map(v => <option key={v} value={v}>{v}</option>)}
                                                </select>
                                            </div>
                                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                                                <div>
                                                    <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Color de cabello</label>
                                                    <select value={veoHairColor} onChange={(e) => setVeoHairColor(e.target.value)} className="input-field">
                                                        {UGC_HAIR_COLORS.map(v => <option key={v} value={v}>{v}</option>)}
                                                    </select>
                                                </div>
                                                <div>
                                                    <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Estilo de cabello</label>
                                                    <select value={veoHairStyle} onChange={(e) => setVeoHairStyle(e.target.value)} className="input-field">
                                                        {UGC_HAIR_STYLES.map(v => <option key={v} value={v}>{v}</option>)}
                                                    </select>
                                                </div>
                                            </div>
                                            <div>
                                                <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Detalles adicionales (opcional)</label>
                                                <textarea
                                                    className="input-field"
                                                    value={veoExtraDetails}
                                                    onChange={(e) => setVeoExtraDetails(e.target.value)}
                                                    placeholder="Ej: pecas, tatuajes, lentes, barba, piercing..."
                                                    style={{ height: 60, fontSize: 12, lineHeight: 1.5 }}
                                                />
                                            </div>
                                        </div>
                                        ...
                                        {/* Paso 1: Guion */}
                                        <h2 style={{ fontSize: 15, fontWeight: 800, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                                            <span style={{ background: "#EC4899", color: "#fff", width: 22, height: 22, borderRadius: "50%", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>1</span> Guion del video
                                        </h2>

                                        <button
                                            type="button"
                                            onClick={generateVideoScript}
                                            disabled={isGeneratingScript || !activeProject?.productPreview}
                                            style={{
                                                width: "100%", padding: "12px 20px", borderRadius: 12,
                                                background: "linear-gradient(90deg, #EC4899, #8B5CF6)", border: "none", color: "#fff",
                                                fontSize: 13, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center",
                                                justifyContent: "center", gap: 8, marginBottom: 16, opacity: (!activeProject?.productPreview) ? 0.5 : 1
                                            }}
                                        >
                                            {isGeneratingScript ? <Loader2 className="animate-spin" size={16} /> : <><Sparkles size={14} /> {videoScript ? "Regenerar guion" : "Generar guion"}</>}
                                        </button>

                                        {(videoScript || isGeneratingScript) && (
                                            <div style={{ marginBottom: 16 }}>
                                                <label style={{ display: "block", fontSize: 11, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", marginBottom: 8 }}>Guion (editable)</label>
                                                <textarea
                                                    className="input-field"
                                                    value={videoScript}
                                                    onChange={(e) => { setVideoScript(e.target.value); setScriptAccepted(false); }}
                                                    placeholder="Aquí aparecerá el guion generado..."
                                                    style={{ height: 160, fontSize: 12, lineHeight: 1.5 }}
                                                />
                                                {!scriptAccepted ? (
                                                    <button
                                                        type="button"
                                                        ...
                                                        style={{ marginTop: 10, width: "100%", padding: "10px 16px", borderRadius: 10, background: "rgba(16, 185, 129, 0.1)", border: "1px solid rgba(16, 185, 129, 0.3)", color: "#10B981", fontSize: 12, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
                                                    >
                                                        <CheckCircle2 size={14} /> Aceptar guion y continuar
                                                    </button>
                                                ) : (
                                                    <div style={{ marginTop: 10, fontSize: 11, color: "#10B981", fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
                                                        <CheckCircle2 size={14} /> Guion aceptado — listo para el Paso 2
                                                    </div>
                                                )}
                                            </div>
                                        )}

                                        {/* Paso 2: Video (solo si el guion fue aceptado) */}
                                        {scriptAccepted && (
                                            <>
                                                <hr style={{ border: "none", borderTop: "1px solid rgba(255,255,255,0.07)", margin: "20px 0" }} />
                                                <h2 style={{ fontSize: 15, fontWeight: 800, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                                                    <span style={{ background: "#EC4899", color: "#fff", width: 22, height: 22, borderRadius: "50%", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>2</span> Generar video
                                                </h2>

                                                <div style={{ background: "rgba(236,72,153,0.06)", border: "1px solid rgba(236,72,153,0.2)", borderRadius: 12, padding: 14, marginBottom: 16 }}>
                                                    <div style={{ fontSize: 12, fontWeight: 800, color: "#EC4899", marginBottom: 4 }}>💳 Costo estimado: ~${VEO_ESTIMATED_COST_USD} USD</div>
                                                    <div style={{ fontSize: 10, color: "#9CA3AF", lineHeight: 1.5 }}>Video de {VEO_CLIP_DURATION_SECONDS}s con Veo Fast (${VEO_PRICE_PER_SECOND_USD}/seg). Estimado según documentación pública de Google — puede variar. Ver ai.google.dev/gemini-api/docs/pricing.</div>
                                                    <div style={{ fontSize: 10, color: "#F59E0B", lineHeight: 1.5, marginTop: 6 }}>⚠️ El video se reproduce directo desde Google, sin guardarlo aparte — deja de funcionar ~48h después de generado. Descárgalo si lo quieres conservar.</div>
                                                </div>

                                                <button
                                                    type="button"
                                                    onClick={handleGenerateVeoVideo}
                                                    disabled={isGeneratingVideo || !activeProject?.productPreview}
                                                    style={{
                                                        width: "100%", padding: "16px 24px", borderRadius: 12,
                                                        background: "linear-gradient(90deg, #EC4899, #F59E0B)", border: "none", color: "#fff",
                                                        fontSize: 15, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center",
                                                        justifyContent: "center", gap: 8, boxShadow: "0 4px 20px rgba(236, 72, 153, 0.25)"
                                                    }}
                                                >
                                                    {isGeneratingVideo ? <Loader2 className="animate-spin" size={18} /> : <><Video size={16} /> Generar Video</>}
                                                </button>
                                            </>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 24 }}>
                                            ...
                                            {(activeProject?.videoResults || []).map((res) => (
                                                <div key={res.id} className="glass-card" style={{ padding: 16, position: "relative", display: "flex", flexDirection: "column" }}>
                                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                                                        <div style={{
                                                            fontSize: 10, fontWeight: 800, padding: "4px 10px", borderRadius: 100, textTransform: "uppercase", letterSpacing: "0.05em",
                                                            background: res.status === 'listo' ? "rgba(16,185,129,0.1)" : res.status === 'error' ? "rgba(239,68,68,0.1)" : "rgba(236,72,153,0.1)",
                                                            color: res.status === 'listo' ? "#10B981" : res.status === 'error' ? "#EF4444" : "#EC4899"
                                                        }}>
                                                            {res.status === 'listo' ? 'Listo' : res.status === 'error' ? 'Error' : 'Generando'}
                                                        </div>
                                                        {res.status === 'listo' && res.videoUrl && (
                                                            <a href={res.videoUrl} download target="_blank" rel="noreferrer" style={{ color: "#EC4899" }}>
                                                                <Download size={16} />
                                                            </a>
                                                        )}
                                                    </div>

                                                    <div style={{ position: "relative", overflow: "hidden", borderRadius: 16, aspectRatio: "9/16", background: "#0d0d14", marginBottom: 12, display: "flex", alignItems: "center", justifyContent: "center" }}>
                                                        {res.status === 'listo' && res.videoUrl ? (
                                                            <video src={res.videoUrl} controls style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                                                        ) : res.status === 'error' ? (
                                                            <div style={{ padding: 20, textAlign: "center" }}>
                                                                <X size={28} style={{ color: "#EF4444", marginBottom: 8 }} />
                                                                <div style={{ fontSize: 11, color: "#F87171" }}>{res.errorMessage || "Error desconocido"}</div>
                                                            </div>
                                                        ) : (
                                                            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: 20, textAlign: "center" }}>
                                                                <Loader2 className="animate-spin" size={32} style={{ color: "#EC4899" }} />
                                                                <span style={{ fontSize: 11, color: "#9CA3AF" }}>Generando video... (puede tardar 1-3 min)</span>
                                                            </div>
                                                        )}
                                                    </div>

                                                    <div style={{ padding: 12, background: "rgba(255,255,255,0.02)", borderRadius: 12, fontSize: 11, color: "#9CA3AF", lineHeight: 1.5, marginTop: "auto", whiteSpace: "pre-wrap" }}>
                                                        {res.script}
                                                    </div>
                                                    ...
                                                    {res.status === 'error' && (
                                                        <button
                                                            type="button"
                                                            onClick={() => { setVideoScript(res.script); setScriptAccepted(true); handleGenerateVeoVideo(); }}
                                                            style={{ marginTop: 10, background: "rgba(236,72,153,0.1)", border: "1px solid rgba(236,72,153,0.2)", color: "#EC4899", padding: "8px 12px", borderRadius: 100, fontSize: 11, fontWeight: 800, cursor: "pointer" }}
                                                        >
                                                            Reintentar
                                                        </button>
                                                    )}
                                            ...
                                            {(!activeProject?.videoResults || activeProject.videoResults.length === 0) && (
                                                <div style={{ gridColumn: "1 / -1", textAlign: "center", padding: "80px 20px" }}>
                                                    <Video size={40} style={{ color: "#EC4899", opacity: 0.25, marginBottom: 16, margin: "0 auto" }} />
                                                    <h3 style={{ fontSize: 16, fontWeight: 700, color: "#E5E7EB", marginBottom: 8 }}>No hay videos generados aún</h3>
                                                    <p style={{ color: "#6B7280", fontSize: 13, maxWidth: 420, margin: "0 auto" }}>Genera y acepta un guion en el panel izquierdo, luego haz clic en "Generar Video".</p>
                                                </div>
                                            )}
