...
export default function Dashboard() {
        ...
        if (user) {
            ...
            try {
                ...
                if (e.name === 'QuotaExceededError' || e.name === 'NS_ERROR_DOM_QUOTA_REACHED') {
                    ...
                    // Level 1: Clear results of OTHER projects
                    let pruned = projects.map(p => p.id === activeProjectId ? p : { ...p, results: [] });

                    try {
                        localStorage.setItem(key, JSON.stringify(pruned));
                        setProjects(pruned);
                        setToast({ msg: "⚠️ Memoria llena: Se han limpiado imágenes de otros proyectos.", type: 'error' });
                        return;
                    } catch (err2) {
                        // Level 2: Limit active project results to last 5 + Clear other projects results
                        pruned = projects.map(p => {
                            if (p.id === activeProjectId) {
                                return { ...p, results: p.results.slice(-5) };
                            }
                            return { ...p, results: [] };
                        });
                        ...
                        try {
                            ...
                            // Also clear library if needed
                            const libKey = getUKey("clickads_library");
                            const savedLib = localStorage.getItem(libKey);
                            if (savedLib) {
                                const libArr = JSON.parse(savedLib);
                                if (libArr.length > 5) {
                                    const prunedLib = libArr.slice(0, 5);
                                    localStorage.setItem(libKey, JSON.stringify(prunedLib));
                                    setLibrary(prunedLib);
                                }
                            }

                            setToast({ msg: "⚠️ Memoria crítica: Se han conservado solo las últimas 5 imágenes.", type: 'error' });
                            return;
                        } catch (err3) {
                            // Level 3: Extreme Pruning - Keep only active project metadata + last 2 images
                            pruned = projects.map(p => {
                                if (p.id === activeProjectId) {
                                    return { ...p, results: p.results.slice(-2) };
                                }
                                return { ...p, results: [], productPreview: "", logoPreview: undefined };
                            });

                            try {
                                localStorage.setItem(key, JSON.stringify(pruned));
                                setProjects(pruned);
                                setToast({ msg: "❌ Memoria CRÍTICA: Se han eliminado proyectos antiguos para poder guardar.", type: 'error' });
                            } catch (finalErr) {
                                setToast({ msg: "❌ ERROR DE ALMACENAMIENTO: Memoria del navegador totalmente llena.", type: 'error' });
                            }
                        }
                    }
                }
    ...
    const createProject = () => {
        ...
        const newProject: Project = {
            ...
            font: selectedFont,
            logoPreview: newProjectLogo || undefined,
            personPreview: newProjectPerson || undefined,
            type: newProjectType
        };
    ...
    const updateActiveProject = (updates: Partial<Project>) => {
        ...
        const newProjects = projects.map(p => {
            if (p.id === activeProjectId) {
                ...
                // Proactive limit: keep only last 15 results per project to save space
                if (merged.results && merged.results.length > 15) {
                    merged.results = merged.results.slice(-15);
                }
                return merged;
            }
            return p;
    ...
    const saveToLibrary = (image: string, angle: string) => {
        ...
        let newLib = [newAd, ...library];

        // Proactive limit: keep only last 20 saved ads to save space
        if (newLib.length > 20) {
            newLib = newLib.slice(0, 20);
        }
    ...
    const downloadSingleImage = (image: string, name: string) => {
        try {
            ...
            // Fallback directo si falla el blob
            const link = document.createElement("a");
            link.href = image;
        ...
        try {
            ...
            for (let i = 0; i < images.length; i++) {
                ...
                if (image.startsWith('data:')) {
                    ...
                    zip.file(`${safeName}_${i + 1}.jpg`, base64Data, { base64: true });
                } else {
                    // Si es una URL externa (menos probable aquí), usamos fetch
                    const response = await fetch(image);
                    const blob = await response.blob();
                    const safeName = name.replace(/[^a-z0-9]/gi, '_').toLowerCase();
                    zip.file(`${safeName}_${i + 1}.jpg`, blob);
                }
            }
    ...
    const handleGenerateClinic = async () => {
        ...
        try {
            const res = await fetch("/api/vertex-ai/clinics", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    beforeBase64: clinicBefore,
                    afterBase64: clinicAfter,
                    treatment: clinicTreatment,
                    apiKey: apiKey,
                    angle: selectedClinicAngle === "ALL" ? undefined : selectedClinicAngle
                }),
            });
            const data = await res.json();
            if (data.error) throw new Error(data.error);
            setClinicResults(data.variations || []);
            setToast({ msg: "¡Creativos clínicos generados!", type: 'success' });
        } catch (error: any) {
            setToast({ msg: error.message, type: 'error' });
        } finally {
    ...
    const handleGenerateDigital = async () => {
        ...
        const productImg = activeProject?.productPreview || digitalProduct;
        const personImg = activeProject?.personPreview || digitalPerson;
        const logoImg = activeProject?.logoPreview || digitalLogo;
        const themePrompt = activeProject?.userPrompt || digitalPrompt;

        if (!productImg) {
            setToast({ msg: "Sube al menos la portada de tu producto", type: 'error' });
            return;
        }
        ...
        try {
            const res = await fetch("/api/vertex-ai/digital", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    personBase64: personImg,
                    productBase64: productImg,
                    logoBase64: logoImg,
                    prompt: themePrompt,
                    apiKey: apiKey,
                    angle: selectedDigitalAngle === "ALL" ? undefined : selectedDigitalAngle,
                    primaryColor: activeProject?.primaryColor || selectedPrimary,
                    secondaryColor: activeProject?.secondaryColor || selectedSecondary,
                    font: activeProject?.font || selectedFont
                }),
            });
            const data = await res.json();
            if (data.error) throw new Error(data.error);

            setDigitalResults(data.variations || []);

            if (activeProjectId && data.variations) {
                const newResults = [...(activeProject?.results || [])];
                data.variations.forEach((v: any) => {
                    newResults.unshift({ image: v.image, title: v.angle });
                });
                updateActiveProject({ results: newResults });
            }

            setToast({ msg: "¡Creativos digitales generados!", type: 'success' });
        } catch (error: any) {
            setToast({ msg: error.message, type: 'error' });
        } finally {
        ...
        <div style={{ background: "#030303", minHeight: "100vh", color: "#fff", fontFamily: "Inter, sans-serif", display: "flex" }}>
            ...
            <aside style={{ width: 280, borderRight: "1px solid rgba(255,255,255,0.05)", padding: "40px 24px", display: "flex", flexDirection: "column", height: "100vh", position: "fixed", background: "#050505" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 48, padding: "0 12px" }}>
                    <div style={{ width: 34, height: 34, background: "linear-gradient(135deg, #8B5CF6, #7C3AED)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 20px rgba(139,92,246,0.2)" }}>
                        <ShieldCheck size={20} color="#fff" />
                    </div>
                    <span style={{ fontSize: 24, fontWeight: 900, letterSpacing: "-1px" }}>SLA</span>
                </div>
                <nav style={{ display: "flex", flexDirection: "column", gap: 6, flex: 1, overflowY: 'auto' }} className="custom-scrollbar">
                    <div className={`nav-item ${activeTab === 'home' ? 'active' : ''}`} onClick={() => setActiveTab('home')}><Layout size={18} /> Dashboard</div>
                    <div className={`nav-item ${activeTab === 'generator' ? 'active' : ''}`} onClick={() => setActiveTab('generator')}><Sparkles size={18} /> Genera Creativos</div>
                    <div className={`nav-item ${activeTab === 'studio' ? 'active' : ''}`} onClick={() => setActiveTab('studio')}><Image size={18} /> Photo Studio</div>
                    <div className={`nav-item ${activeTab === 'library' ? 'active' : ''}`} onClick={() => setActiveTab('library')}><Bookmark size={18} /> Mi Biblioteca</div>
                    <div className={`nav-item ${activeTab === 'course' ? 'active' : ''}`} onClick={() => setActiveTab('course')}><BookOpen size={18} /> Curso</div>
                    <div className={`nav-item ${activeTab === 'calendar' ? 'active' : ''}`} onClick={() => setActiveTab('calendar')}><Calendar size={18} /> Calendario</div>
            ...
            <main style={{ marginLeft: 300, flex: 1, padding: "40px 60px", overflowX: 'hidden' }}>
                ...
                {activeTab === 'home' && (() => {
                    ...
                    const recentLessons = [
                        { id: 1, title: 'Tracking Avanzado, creando y configurando UTMnify', sub: 'Meta Ads por Santiago Bilbao · Tracking Avanzado', icon: <PlayCircle size={18} color="#8B5CF6" /> },
                        { id: 2, title: 'SetUp Campaña Desde 0', sub: 'Meta Ads por Santiago Bilbao · Meta Ads', icon: <PlayCircle size={18} color="#8B5CF6" /> },
                        { id: 3, title: 'Apalancate de lo que ya funciona', sub: 'Meta Ads por Santiago Bilbao · Creativos', icon: <PlayCircle size={18} color="#8B5CF6" /> },
                        { id: 4, title: 'Imagenes, Videos o Carruseles', sub: 'Meta Ads por Santiago Bilbao · Creativos', icon: <PlayCircle size={18} color="#8B5CF6" /> },
                        { id: 5, title: 'Clase más importante. ¿Como Testear y Escalar?', sub: 'Meta Ads por Santiago Bilbao · Meta Ads', icon: <PlayCircle size={18} color="#8B5CF6" /> },
                    ];

                    const myCourses = [
                        { id: 1, name: 'Apps de IA por Claudio Conde', lessons: '0 de 10 lecciones', progress: 0, img: 'https://images.unsplash.com/photo-1677442136019-21780ecad995?auto=format&fit=crop&q=80&w=200' },
                        { id: 2, name: 'Meta Ads por Santiago Bilbao', lessons: '0 de 17 lecciones', progress: 0, img: 'https://images.unsplash.com/photo-1611162617474-5b21e879e113?auto=format&fit=crop&q=80&w=200' },
                    ];

                    return (
                        <div style={{ maxWidth: 1200, margin: '0 auto' }} className="anim-in">
                            {/* Breadcrumbs */}
                            <div style={{ display: 'flex', gap: 8, fontSize: 13, color: '#6B7280', marginBottom: 32, fontWeight: 600 }}>
                                <span>Inicio</span>
                                <span style={{ opacity: 0.5 }}>Dashboard</span>
                            </div>

                            {/* Hero Greeting Card */}
                            <div className="hero-card" style={{ marginBottom: 40, background: '#0A0A0A', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 24, padding: '48px' }}>
                                <div style={{ display: 'flex', gap: 24, alignItems: 'center', marginBottom: 48 }}>
                                    <div style={{ width: 68, height: 68, background: '#8B5CF6', borderRadius: '50%', flexShrink: 0, boxShadow: '0 0 30px rgba(139, 92, 246, 0.15)' }} />
                                    <div>
                                        <h1 style={{ fontSize: 38, fontWeight: 900, margin: 0, letterSpacing: '-1.5px', lineHeight: 1 }}>{greeting}, {user?.name?.split(' ')[0] || 'Claudio'}</h1>
                                        <p style={{ color: '#6B7280', margin: '8px 0 0', fontWeight: 600, fontSize: 15 }}>{dateStr}</p>
                                        <p style={{ color: '#8B5CF6', fontSize: 14, fontWeight: 700, marginTop: 12 }}>Seguí avanzando en tu formación</p>
                                    </div>
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 40, paddingTop: 40, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                                    <div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 12, color: '#8B5CF6', marginBottom: 6 }}>
                                            <CheckCircle2 size={22} />
                                            <span style={{ fontSize: 30, fontWeight: 900 }}>0/17</span>
                                        </div>
                                        <div style={{ fontSize: 12, color: '#4B5563', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px' }}>Lecciones</div>
                                    </div>
                                    <div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 12, color: '#8B5CF6', marginBottom: 6 }}>
                                            <TrendingUp size={22} />
                                            <span style={{ fontSize: 30, fontWeight: 900 }}>0 días</span>
                                        </div>
                                        <div style={{ fontSize: 12, color: '#4B5563', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px' }}>Racha</div>
                                    </div>
                                    <div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 12, color: '#8B5CF6', marginBottom: 6 }}>
                                            <BookOpen size={22} />
                                            <span style={{ fontSize: 30, fontWeight: 900 }}>17</span>
                                        </div>
                                        <div style={{ fontSize: 12, color: '#4B5563', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px' }}>Restantes</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 32, marginBottom: 40 }}>
                                ...
                                <div style={{ background: '#0A0A0A', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 24, padding: 32 }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
                                        <h2 style={{ fontSize: 20, fontWeight: 800, margin: 0 }}>Lecciones Recientes</h2>
                                        <span style={{ color: '#8B5CF6', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>Ver todo</span>
                                    </div>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        {recentLessons.map(lesson => (
                                            <div key={lesson.id} className="lesson-item">
                                                <div style={{ width: 44, height: 44, borderRadius: 14, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                                    {lesson.icon}
                                                </div>
                                                <div style={{ flex: 1 }}>
                                                    <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 2 }}>{lesson.title}</div>
                                                    <div style={{ fontSize: 12, color: '#4B5563', fontWeight: 600 }}>{lesson.sub}</div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div style={{ background: '#0A0A0A', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 24, padding: 32 }}>
                                    <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 32 }}>
                                        ...
                                        <h2 style={{ fontSize: 18, fontWeight: 800, margin: 0 }}>Actividad Semanal</h2>
                                    </div>
                                    <div style={{ height: 180, display: 'flex', alignItems: 'flex-end', gap: 12, paddingBottom: 20 }}>
                                        {['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'].map((day, i) => (
                                            <div key={day} style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12, height: '100%' }}>
                                                <div style={{ background: 'rgba(255,255,255,0.02)', borderRadius: 6, flex: 1, position: 'relative', overflow: 'hidden' }}>
                                                    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: i === 0 ? '25%' : '8%', background: i === 0 ? '#8B5CF6' : 'rgba(255,255,255,0.05)', borderRadius: 4 }} />
                                                </div>
                                                <div style={{ fontSize: 11, color: '#4B5563', textAlign: 'center', fontWeight: 800 }}>{day}</div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
                                {/* Live Class Card */}
                                <div style={{ background: '#0A0A0A', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 24, padding: 32 }}>
                                    <h2 style={{ fontSize: 20, fontWeight: 800, margin: '0 0 32px' }}>Próxima Clase en Vivo</h2>
                                    <div style={{ display: 'flex', gap: 20, alignItems: 'center', marginBottom: 32 }}>
                                        <div style={{ width: 56, height: 56, borderRadius: 16, background: 'rgba(139, 92, 246, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(139, 92, 246, 0.2)' }}>
                                            <Video size={28} color="#8B5CF6" />
                                        </div>
                                        <div>
                                            <div style={{ fontSize: 18, fontWeight: 900, marginBottom: 4 }}>Mentoría Avanzada</div>
                                            <div style={{ fontSize: 14, color: '#6B7280', fontWeight: 600 }}>Mentoría semanal para alumnos avanzados.</div>
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 32, padding: '16px', background: 'rgba(255,255,255,0.02)', borderRadius: 16, border: '1px solid rgba(255,255,255,0.05)' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, fontWeight: 800, color: '#8B5CF6' }}>
                                            <Calendar size={18} />
                                            En 3 días
                                        </div>
                                        <div style={{ width: 1, height: 16, background: 'rgba(255,255,255,0.1)' }} />
                                        <div style={{ color: '#6B7280', fontSize: 13, fontWeight: 700 }}>21 de mar de 2026 · 90 min</div>
                                    </div>
                                    <button className="btn-purple" style={{ padding: '18px' }}>Unirse a la llamada</button>
                                </div>

                                {/* My Courses */}
                                <div style={{ background: '#0A0A0A', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 24, padding: 32 }}>
                                    <h2 style={{ fontSize: 20, fontWeight: 800, margin: '0 0 32px' }}>Mis Cursos</h2>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                                        {myCourses.map(course => (
                                            <div key={course.id} style={{ background: 'rgba(255,255,255,0.02)', borderRadius: 20, padding: 20, border: '1px solid rgba(255,255,255,0.05)', transition: '0.2s', cursor: 'pointer' }} className="hover-scale">
                                                <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginBottom: 20 }}>
                                                    <img src={course.img} style={{ width: 48, height: 48, borderRadius: 12, objectFit: 'cover', border: '1px solid rgba(255,255,255,0.1)' }} />
                                                    <div style={{ overflow: 'hidden' }}>
                                                        <div style={{ fontSize: 13, fontWeight: 900, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: 2 }}>{course.name}</div>
                                                        <div style={{ fontSize: 11, color: '#555', fontWeight: 700 }}>{course.lessons}</div>
                                                    </div>
                                                </div>
                                                <div className="progress-bar" style={{ marginBottom: 12 }}>
                                                    <div className="progress-fill" style={{ width: `${course.progress}%` }} />
                                                </div>
                                                <div style={{ textAlign: 'right', fontSize: 16, fontWeight: 900, color: '#444' }}>{course.progress}%</div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                                ...
                                                <div key={i} className="glass-card" style={{ padding: 12, position: "relative" }}>
                                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                                                        ...
                                                        <div style={{ display: "flex", gap: 12 }}>
                                                            <button
                                                                onClick={() => handleGenerateCopy(i)}
                                                                disabled={generatingCopyIndex === i}
                                                                style={{ background: "none", border: "none", color: activeProject.primaryColor, cursor: "pointer", display: "flex", alignItems: "center" }}
                                                                title="Generar Copy"
                                                            >
                                                                {generatingCopyIndex === i ? <Loader2 size={16} className="animate-spin" /> : <MessageSquare size={16} />}
                                                            </button>
                                                    ...
                                                    {res.copy && (
                                                        <div style={{ marginTop: 12, padding: 12, background: "rgba(255,255,255,0.03)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.05)" }}>
                                                            <div style={{ fontSize: 10, fontWeight: 900, color: activeProject.primaryColor, marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                                                AD COPY SUGERIDO
                                                                <button
                                                                    onClick={() => {
                ...
                {activeTab === 'calendar' && (
                    <div style={{ height: "70vh", display: "flex", flexDirection: 'column', alignItems: "center", justifyContent: "center", gap: 24, opacity: 0.5 }}>
                        <Calendar size={80} color="#EAB308" />
                        <div style={{ textAlign: "center" }}>
                            <h2 style={{ fontSize: 24, fontWeight: 900, marginBottom: 8 }}>Calendario de Eventos</h2>
                            <p style={{ fontSize: 16, fontWeight: 600 }}>Próximamente: Sincroniza tus mentorías y clases en vivo.</p>
                        </div>
                    </div>
                )}
