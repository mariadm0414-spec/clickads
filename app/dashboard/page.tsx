...
export default function Dashboard() {
    ...
    const [ugcAngle, setUgcAngle] = useState("Natural/Testimonio");
    const [ugcResult, setUgcResult] = useState("");
    const [isUgcLoading, setIsUgcLoading] = useState(false);
    ...
    // WhatsApp Closer States
    const [waTabMode, setWaTabMode] = useState<'analyze' | 'simulate'>('analyze');
    const [waChatInput, setWaChatInput] = useState("");
    const [waAnalysisResult, setWaAnalysisResult] = useState<any>(null);
    const [isWaAnalyzing, setIsWaAnalyzing] = useState(false);
    const [waClientType, setWaClientType] = useState("cliente frío (solo pregunta precio)");
    ...
    const [waSimEvaluation, setWaSimEvaluation] = useState<any>(null);
...
Escribe un copy que conecte perfectamente con la nueva imagen del anuncio. Usa una estructura persuasiva y ganchos atractivos. Responde SOLO el texto del copy.` }
    ...
    const handleGenerateUgc = async () => {
        if (!apiKey) {
            setToast({ msg: "Configura tu API Key en la sección 'Motor' primero", type: 'error' });
            return;
        }
        if (!ugcProductName.trim()) {
            setToast({ msg: "Ingresa el nombre del producto", type: 'error' });
            return;
        }
        setIsUgcLoading(true);
        setUgcResult("");
        try {
            const response = await fetch("/api/vertex-ai/generate-ugc-copy", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    ...
                    angle: ugcAngle,
                    language: "Español",
                    apiKey: apiKey // Added this
                })
            });
            const data = await response.json();
            if (data.success) {
                setUgcResult(data.copy);
                setToast({ msg: "¡Guion UGC Premium generado!", type: 'success' });
            } else {
                throw new Error(data.error || "Error generando guion");
            }
        } catch (e: any) {
            setToast({ msg: e.message || "Error al generar guion", type: 'error' });
        } finally {
            setIsUgcLoading(false);
        }
    };

    const handleDestroyObjections = async () => {
        if (!apiKey) {
            setToast({ msg: "Configura tu API Key en la sección 'Motor' primero", type: 'error' });
            return;
        }
        ...
        setObjectionResult("");
        try {
            const response = await fetch("/api/vertex-ai/objections", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    productName: objectionProductName,
            ...
            if (data.success) {
                ...
                setToast({ msg: "¡Objeciones destruidas!", type: 'success' });
            } else {
                throw new Error(data.error || "Error procesando objeciones");
            }
        } catch (e: any) {
            setToast({ msg: e.message || "Error al procesar", type: 'error' });
        } finally {
    ...
    const handleWaAnalyze = async () => {
        if (!apiKey) return setToast({ msg: "Configura tu API Key primero", type: 'error' });
        if (!waChatInput.trim()) return setToast({ msg: "Pega una conversación", type: 'error' });
        setIsWaAnalyzing(true);
        setWaAnalysisResult(null);
        try {
            const res = await fetch("/api/vertex-ai/whatsapp-closer", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ mode: "analyze", userMessage: waChatInput, apiKey: apiKey })
            });
            const data = await res.json();
            if (data.success) {
                setWaAnalysisResult(data.analysis);
                setToast({ msg: "Análisis completado", type: 'success' });
            } else throw new Error(data.error);
        } catch (e: any) {
            setToast({ msg: e.message || "Error al analizar", type: 'error' });
        } finally {
            setIsWaAnalyzing(false);
        }
    };
        ...
        try {
            const res = await fetch("/api/vertex-ai/whatsapp-closer", {
                ...
                body: JSON.stringify({
                    ...
                    productName: activeProject?.productName || "un producto increíble",
                    targetAudience: activeProject?.targetAudience || "un cliente interesado",
                    userMessage: "Hola, soy una persona interesada en comprar esto. Inicia la conversación con un saludo natural de cliente que quiere información.",
                    apiKey: apiKey
                })
            });
            const data = await res.json();
            ...
            setToast({ msg: "Error al iniciar simulación", type: 'error' });
        } finally {
            setIsWaSimLoading(false);
        }
    };

    const handleWaSimSendMessage = async () => {
        if (!waSimInput.trim() || isWaSimLoading) return;
        const msg = waSimInput;
        setWaSimInput("");
        const newUserMsg = { role: 'user' as const, content: msg };
        const newHistory = [...waSimMessages, newUserMsg];
        setWaSimMessages(newHistory);
        setIsWaSimLoading(true);
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                setWaSimMessages(prev => [...prev, { role: 'assistant', content: data.response }]);
            }
        } catch (e) {
            setToast({ msg: "Error en simulación", type: 'error' });
        } finally {
            setIsWaSimLoading(false);
        }
    };
        ...
        <div style={{ background: "#030303", minHeight: "100vh", color: "#fff", fontFamily: "Inter, sans-serif", display: "flex" }}>
            ...
            <aside style={{ width: 280, borderRight: "1px solid rgba(255,255,255,0.05)", padding: "40px 24px", display: "flex", flexDirection: "column", height: "100vh", position: "fixed", background: "#050505" }}>
                ...
                <nav style={{ display: "flex", flexDirection: "column", gap: 6, flex: 1, overflowY: 'auto' }} className="custom-scrollbar">
                    ...
                    <div className={`nav-item ${activeTab === 'ugc_scripts' ? 'active' : ''}`} onClick={() => setActiveTab('ugc_scripts')} style={{ background: activeTab === 'ugc_scripts' ? 'linear-gradient(135deg,rgba(139,92,246,0.25),rgba(139,92,246,0.15))' : '', color: activeTab === 'ugc_scripts' ? '#A78BFA' : '', border: activeTab === 'ugc_scripts' ? '1px solid rgba(139,92,246,0.3)' : '1px solid transparent' }}>
                        <Video size={18} /> Guiones Premium UGC
                        <div style={{ marginLeft: "auto", background: "#facc15", color: "#000", padding: "2px 6px", borderRadius: 6, fontSize: 8, fontWeight: 900 }}>PRO</div>
                    </div>
                    <div className={`nav-item ${activeTab === 'objections' ? 'active' : ''}`} onClick={() => setActiveTab('objections')} style={{ background: activeTab === 'objections' ? 'linear-gradient(135deg,rgba(239,68,68,0.25),rgba(239,68,68,0.15))' : '', color: activeTab === 'objections' ? '#f87171' : '', border: activeTab === 'objections' ? '1px solid rgba(239,68,68,0.3)' : '1px solid transparent' }}>
                        <ShieldAlert size={18} /> Destruir Objeciones
                        <div style={{ marginLeft: "auto", background: "#ef4444", color: "#fff", padding: "2px 6px", borderRadius: 6, fontSize: 8, fontWeight: 900 }}>HOT</div>
                    </div>
                    <div className={`nav-item ${activeTab === 'research' ? 'active' : ''}`} onClick={() => setActiveTab('research')} style={{ position: "relative" }}>
                        <Search size={18} /> Investigación de Producto
                        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 4, background: "linear-gradient(135deg,rgba(234,179,8,0.2),rgba(202,138,4,0.1))", border: "1px solid rgba(234,179,8,0.3)", color: "#facc15", padding: "3px 6px", borderRadius: 8, fontSize: 9, fontWeight: 900, letterSpacing: 0.5, boxShadow: "0 0 10px rgba(234,179,8,0.2)" }}>
                            <Sparkles size={10} color="#facc15" /> NUEVO
                        </div>
                    </div>
                    <div className={`nav-item ${activeTab === 'whatsapp_closer' ? 'active' : ''}`} onClick={() => setActiveTab('whatsapp_closer')} style={{ background: activeTab === 'whatsapp_closer' ? 'linear-gradient(135deg,rgba(37,211,102,0.25),rgba(37,211,102,0.15))' : '', color: activeTab === 'whatsapp_closer' ? '#25D366' : '', border: activeTab === 'whatsapp_closer' ? '1px solid rgba(37,211,102,0.3)' : '1px solid transparent' }}>
                        <MessageSquare size={18} /> Vende + con WhatsApp
                        <div style={{ marginLeft: "auto", background: "#25D366", color: "#000", padding: "2px 6px", borderRadius: 6, fontSize: 8, fontWeight: 900 }}>NUEVO</div>
                    </div>
            ...
            <main style={{ marginLeft: 300, flex: 1, padding: "40px 60px", overflowX: 'hidden' }}>
                ...
                {activeTab === 'ugc_scripts' && (
                    <div style={{ maxWidth: 1100 }} className="anim-in">
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 40 }}>
                            <div>
                                <h1 style={{ fontSize: 36, fontWeight: 900, background: "linear-gradient(to right, #fff, #8B5CF6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", marginBottom: 8 }}>Guiones Premium UGC</h1>
                                <p style={{ color: "#9CA3AF", fontSize: 16 }}>Genera guiones persuasivos y virales para tus anuncios de video.</p>
                            </div>
                            <div style={{ background: "rgba(250, 204, 21, 0.1)", border: "1px solid rgba(250, 204, 21, 0.2)", padding: "12px 24px", borderRadius: 16, display: "flex", alignItems: "center", gap: 12 }}>
                                <Award color="#facc15" size={24} />
                                <div>
                                    <div style={{ color: "#facc15", fontWeight: 900, fontSize: 12 }}>CALIDAD PREMIUM</div>
                                    <div style={{ color: "#9CA3AF", fontSize: 10 }}>Optimizado para conversiones hgh-ticket</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: 32 }}>
                            <div className="glass-card" style={{ padding: 32, height: "fit-content" }}>
                                <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                                    <div>
                                        <label style={{ display: "block", fontSize: 12, fontWeight: 800, color: "#8B5CF6", marginBottom: 10, textTransform: "uppercase", letterSpacing: 1 }}>Nombre del Producto</label>
                                        <input
                                            type="text"
                                            value={ugcProductName}
                                            onChange={(e) => setUgcProductName(e.target.value)}
                                            placeholder="Ej: Sérum Facial Rejuvenecedor"
                                            style={{ width: "100%", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "14px 18px", color: "#fff", fontSize: 14 }}
                                        />
                                    </div>

                                    <div>
                                        <label style={{ display: "block", fontSize: 12, fontWeight: 800, color: "#8B5CF6", marginBottom: 10, textTransform: "uppercase", letterSpacing: 1 }}>Público Objetivo</label>
                                        <input
                                            type="text"
                                            value={ugcTargetAudience}
                                            onChange={(e) => setUgcTargetAudience(e.target.value)}
                                            placeholder="Ej: Mujeres de 35-50 años interesadas en skincare"
                                            style={{ width: "100%", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "14px 18px", color: "#fff", fontSize: 14 }}
                                        />
                                    </div>

                                    <div>
                                        <label style={{ display: "block", fontSize: 12, fontWeight: 800, color: "#8B5CF6", marginBottom: 10, textTransform: "uppercase", letterSpacing: 1 }}>Estilo / Ángulo del Guion</label>
                                        <select
                                            value={ugcAngle}
                                            onChange={(e) => setUgcAngle(e.target.value)}
                                            style={{ width: "100%", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "14px 18px", color: "#fff", fontSize: 14, outline: "none" }}
                                        >
                                            <option value="Natural/Testimonio">Natural / Testimonio Real</option>
                                            <option value="Problema/Solucion">Problema / Solución Directa</option>
                                            <option value="Review Educativo">Review / Unboxing Educativo</option>
                                            <option value="Trend Viral">Trend / Estilo TikTok Viral</option>
                                            <option value="Comparativo">Comparativo / Antes vs Después</option>
                                        </select>
                                    </div>

                                    <button
                                        className="btn-primary"
                                        style={{ width: "100%", height: 56, justifyContent: "center", background: "linear-gradient(135deg, #8B5CF6 0%, #7C3AED 100%)", boxShadow: "0 10px 20px rgba(139, 92, 246, 0.2)", marginTop: 8 }}
                                        onClick={handleGenerateUgc}
                                        disabled={isUgcLoading}
                                    >
                                        {isUgcLoading ? <Loader2 className="animate-spin" /> : <><Sparkles size={20} /> Generar Guion Maestro</>}
                                    </button>
                                </div>
                            </div>

                            <div className="glass-card" style={{ padding: 40, position: "relative", minHeight: 500 }}>
                                {!ugcResult && !isUgcLoading && (
                                    <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: 0.2, textAlign: "center" }}>
                                        <Video size={80} style={{ marginBottom: 24 }} />
                                        <p style={{ fontSize: 18, fontWeight: 600 }}>Tu guion aparecerá aquí...</p>
                                        <p style={{ fontSize: 14 }}>Rellena los datos y deja que la IA trabaje</p>
                                    </div>
                                )}

                                {isUgcLoading && (
                                    <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
                                        <div className="animate-spin" style={{ width: 40, height: 40, border: "3px solid rgba(139, 92, 246, 0.1)", borderTop: "3px solid #8B5CF6", borderRadius: "50%", marginBottom: 24 }}></div>
                                        <p style={{ fontSize: 18, fontWeight: 700, color: "#8B5CF6" }}>Redactando Guion Maestro...</p>
                                        <p style={{ color: "#9CA3AF", fontSize: 14 }}>Aplicando gatillos mentales de alta conversión</p>
                                    </div>
                                )}
                                    ...
                                    <div className="anim-in">
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                                ...
                                                <span style={{ fontWeight: 800, fontSize: 14 }}>GUION LISTO</span>
                                            </div>
                                            <div style={{ display: "flex", gap: 12 }}>
                                                <button
                                                    onClick={() => {
                                                        navigator.clipboard.writeText(ugcResult);
                                                        setToast({ msg: "Guion copiado al portapapeles", type: 'success' });
                                                    }}
                                                    style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", padding: "8px 16px", borderRadius: 10, cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", gap: 8 }}
                                                >
                                                    <Layers size={14} /> Copiar Todo
                                                </button>
                                            </div>
                                        </div>
                                        <div style={{ background: "rgba(0,0,0,0.2)", padding: 24, borderRadius: 16, border: "1px solid rgba(255,255,255,0.05)", maxHeight: "60vh", overflowY: "auto" }} className="custom-scrollbar">
                                            <div style={{ whiteSpace: "pre-wrap", fontSize: 15, lineHeight: 1.8, color: "#E5E7EB", fontStyle: "italic" }}>
                                                {ugcResult}
                                            </div>
                                        </div>
                                        <div style={{ marginTop: 24, padding: 20, background: "rgba(139, 92, 246, 0.05)", borderRadius: 16, border: "1px dashed rgba(139, 92, 246, 0.2)" }}>
                                            <p style={{ fontSize: 12, color: "#A78BFA", fontWeight: 700, marginBottom: 8, textTransform: "uppercase" }}>Tip de Experto:</p>
                                            <p style={{ fontSize: 13, color: "#9CA3AF", margin: 0 }}>
                                                Usa este guion con un creador real para obtener los mejores resultados. Asegúrate de que el hook (primeros 3 segundos) sea impactante visualmente.
                                            </p>
                                        </div>
                                    </div>
                                )}
                ...
                {activeTab === 'objections' && (
                    <div style={{ maxWidth: 1100 }} className="anim-in">
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 40 }}>
                            <div>
                                <h1 style={{ fontSize: 36, fontWeight: 900, background: "linear-gradient(to right, #fff, #ef4444)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", marginBottom: 8 }}>Destruir Objeciones</h1>
                                <p style={{ color: "#9CA3AF", fontSize: 16 }}>Aniquila las dudas de tus clientes con argumentos psicológicos demoledores.</p>
                            </div>
                            <div style={{ background: "rgba(239, 68, 68, 0.1)", border: "1px solid rgba(239, 68, 68, 0.2)", padding: "12px 24px", borderRadius: 16, display: "flex", alignItems: "center", gap: 12 }}>
                                <ShieldCheck color="#ef4444" size={24} />
                                <div>
                                    <div style={{ color: "#ef4444", fontWeight: 900, fontSize: 12 }}>MODO ASALTO</div>
                                    <div style={{ color: "#9CA3AF", fontSize: 10 }}>Persuasión nivel neuro-marketing</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: 32 }}>
                            <div className="glass-card" style={{ padding: 32, height: "fit-content" }}>
                                <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                                    <div>
                                        <label style={{ display: "block", fontSize: 12, fontWeight: 800, color: "#ef4444", marginBottom: 10, textTransform: "uppercase", letterSpacing: 1 }}>Nombre del Producto</label>
                                        <input
                                            type="text"
                                            value={objectionProductName}
                                            onChange={(e) => setObjectionProductName(e.target.value)}
                                            placeholder="Ej: Curso de Trading para Principiantes"
                                            style={{ width: "100%", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "14px 18px", color: "#fff", fontSize: 14 }}
                                        />
                                    </div>

                                    <div>
                                        <label style={{ display: "block", fontSize: 12, fontWeight: 800, color: "#ef4444", marginBottom: 10, textTransform: "uppercase", letterSpacing: 1 }}>Objeciones Específicas (Opcional)</label>
                                        <textarea
                                            value={objectionList}
                                            onChange={(e) => setObjectionList(e.target.value)}
                                            placeholder="Ej: Es muy caro, no tengo tiempo, no sé si me servirá..."
                                            style={{ width: "100%", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "14px 18px", color: "#fff", fontSize: 14, minHeight: 120, resize: "none" }}
                                        />
                                        <p style={{ fontSize: 11, color: "#6B7280", marginTop: 8 }}>Si lo dejas vacío, la IA generará las objeciones más comunes y las destruirá.</p>
                                    </div>

                                    <button
                                        className="btn-primary"
                                        style={{ width: "100%", height: 56, justifyContent: "center", background: "linear-gradient(135deg, #ef4444 0%, #b91c1c 100%)", boxShadow: "0 10px 20px rgba(239, 68, 68, 0.2)", marginTop: 8 }}
                                        onClick={handleDestroyObjections}
                                        disabled={isObjectionLoading}
                                    >
                                        {isObjectionLoading ? <Loader2 className="animate-spin" /> : <><ShieldAlert size={20} /> Aniquilar Objeciones</>}
                                    </button>
                                </div>
                            </div>

                            <div className="glass-card" style={{ padding: 40, position: "relative", minHeight: 500 }}>
                                {!objectionResult && !isObjectionLoading && (
                                    <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: 0.2, textAlign: "center" }}>
                                        <ShieldCheck size={80} style={{ marginBottom: 24 }} />
                                        <p style={{ fontSize: 18, fontWeight: 600 }}>Tus argumentos aparecerán aquí...</p>
                                        <p style={{ fontSize: 14 }}>Prepara tus defensas y ataca las dudas</p>
                                    </div>
                                )}

                                {isObjectionLoading && (
                                    <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
                                        <div className="animate-spin" style={{ width: 40, height: 40, border: "3px solid rgba(239, 68, 68, 0.1)", borderTop: "3px solid #ef4444", borderRadius: "50%", marginBottom: 24 }}></div>
                                        <p style={{ fontSize: 18, fontWeight: 700, color: "#ef4444" }}>Analizando psicología del consumidor...</p>
                                        <p style={{ color: "#9CA3AF", fontSize: 14 }}>Fabricando respuestas demoledoras</p>
                                    </div>
                                )}
                                    ...
                                    <div className="anim-in">
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                                ...
                                                <span style={{ fontWeight: 800, fontSize: 14 }}>OBJECIONES DESTRUIDAS</span>
                                            </div>
                                            <div style={{ display: "flex", gap: 12 }}>
                                                <button
                                                    onClick={() => {
                                                        navigator.clipboard.writeText(objectionResult);
                                                        setToast({ msg: "Copiado al portapapeles", type: 'success' });
                                                    }}
                                                    style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", padding: "8px 16px", borderRadius: 10, cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", gap: 8 }}
                                                >
                                                    <Layers size={14} /> Copiar Todo
                                        ...
                                        <div style={{ background: "rgba(0,0,0,0.2)", padding: 24, borderRadius: 16, border: "1px solid rgba(255,255,255,0.05)", maxHeight: "60vh", overflowY: "auto" }} className="custom-scrollbar">
                                            <div style={{ whiteSpace: "pre-wrap", fontSize: 15, lineHeight: 1.8, color: "#FEE2E2" }}>
                                                {objectionResult}
                                            </div>
                                        </div>
                                        <div style={{ marginTop: 24, padding: 20, background: "rgba(239, 68, 68, 0.05)", borderRadius: 16, border: "1px dashed rgba(239, 68, 68, 0.2)" }}>
                                            <p style={{ fontSize: 12, color: "#f87171", fontWeight: 700, marginBottom: 8, textTransform: "uppercase" }}>Consejo de Cierre:</p>
                                            <p style={{ fontSize: 13, color: "#9CA3AF", margin: 0 }}>
                                                Usa estas respuestas en tu sección de FAQs o durante tu pitch de ventas para eliminar la fricción antes de que el cliente la mencione.
                                            </p>
                                        </div>
                                    </div>
                                )}
                ...
                {activeTab === 'whatsapp_closer' && (
                    <div style={{ maxWidth: 1200 }} className="anim-in">
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
                            <div>
                                <h1 style={{ fontSize: 36, fontWeight: 900, background: "linear-gradient(to right, #fff, #25D366)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", marginBottom: 8 }}>Vende + con WhatsApp</h1>
                                <p style={{ color: "#9CA3AF", fontSize: 16 }}>Tu closer digital personal. Analiza chats o entrena tus cierres.</p>
                            </div>
                            <div style={{ display: "flex", gap: 12, background: "rgba(255,255,255,0.03)", padding: 6, borderRadius: 16, border: "1px solid rgba(255,255,255,0.05)" }}>
                                <button
                                    onClick={() => setWaTabMode('analyze')}
                                    style={{ padding: "10px 20px", borderRadius: 12, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 13, background: waTabMode === 'analyze' ? "#25D366" : "transparent", color: waTabMode === 'analyze' ? "#000" : "#9CA3AF", transition: "0.2s" }}
                                >Analizador de Chat</button>
                                <button
                                    onClick={() => setWaTabMode('simulate')}
                                    style={{ padding: "10px 20px", borderRadius: 12, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 13, background: waTabMode === 'simulate' ? "#25D366" : "transparent", color: waTabMode === 'simulate' ? "#000" : "#9CA3AF", transition: "0.2s" }}
                                >Simular Conversación</button>
                            </div>
                        </div>
                        ...
                        {waTabMode === 'analyze' ? (
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr", gap: 32 }}>
                                <div className="glass-card" style={{ padding: 32, height: "fit-content" }}>
                                    <label style={{ display: "block", fontSize: 12, fontWeight: 800, color: "#25D366", marginBottom: 16, textTransform: "uppercase", letterSpacing: 1 }}>Pega la conversación aquí</label>
                                    <textarea
                                        value={waChatInput}
                                        onChange={(e) => setWaChatInput(e.target.value)}
                                        placeholder="Cliente: Hola, ¿qué precio tiene?&#10;Vendedor: Hola, ¿cómo estás? Cuesta..."
                                        style={{ width: "100%", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, padding: "18px", color: "#fff", fontSize: 14, minHeight: 400, resize: "none", marginBottom: 24, lineHeight: 1.6 }}
                                    />
                                    <button
                                        className="btn-primary"
                                        style={{ width: "100%", height: 56, justifyContent: "center", background: "#25D366", color: "#000" }}
                                        onClick={handleWaAnalyze}
                                        disabled={isWaAnalyzing}
                                    >
                                        {isWaAnalyzing ? <Loader2 className="animate-spin" /> : <><Zap size={20} /> Analizar Chat Ahora</>}
                                    </button>
                                </div>

                                <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                                    {!waAnalysisResult && !isWaAnalyzing && (
                                        <div className="glass-card" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: 0.3, minHeight: 600 }}>
                                            <MessageSquare size={64} style={{ marginBottom: 24 }} />
                                            <p style={{ fontSize: 18, fontWeight: 600 }}>Esperando conversación...</p>
                                        </div>
                                    )}

                                    {isWaAnalyzing && (
                                        <div className="glass-card" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 600 }}>
                                            <div className="animate-spin" style={{ width: 48, height: 48, border: "3px solid rgba(37, 211, 102, 0.1)", borderTop: "3px solid #25D366", borderRadius: "50%", marginBottom: 24 }}></div>
                                            <p style={{ fontSize: 18, fontWeight: 700, color: "#25D366" }}>Hackeando la mente del cliente...</p>
                                        </div>
                                    )}
                                        ...
                                        <div className="anim-in" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 20 }}>
                                                <div className="glass-card" style={{ padding: 20, textAlign: "center" }}>
                                                    <div style={{ fontSize: 10, color: "#9CA3AF", marginBottom: 8, fontWeight: 800 }}>ESTADO</div>
                                                    <div style={{ fontSize: 18, color: "#25D366", fontWeight: 900 }}>{waAnalysisResult.status}</div>
                                                </div>
                                                <div className="glass-card" style={{ padding: 20, textAlign: "center" }}>
                                                    <div style={{ fontSize: 10, color: "#9CA3AF", marginBottom: 8, fontWeight: 800 }}>INTENCIÓN</div>
                                                    <div style={{ fontSize: 18, color: "#25D366", fontWeight: 900 }}>{waAnalysisResult.intent}</div>
                                                </div>
                                                <div className="glass-card" style={{ padding: 20, textAlign: "center" }}>
                                                    <div style={{ fontSize: 10, color: "#9CA3AF", marginBottom: 8, fontWeight: 800 }}>CONV. SCORE</div>
                                                    <div style={{ fontSize: 18, color: "#25D366", fontWeight: 900 }}>{waAnalysisResult.score}/100</div>
                                                </div>
                                            </div>

                                            <div className="glass-card" style={{ padding: 32 }}>
                                                <h3 style={{ fontSize: 18, fontWeight: 900, marginBottom: 24, color: "#25D366", display: "flex", alignItems: "center", gap: 10 }}>
                                                    <CheckCircle2 size={20} /> Respuestas Sugeridas
                                                </h3>
                                                <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                                                    <div style={{ background: "rgba(37,211,102,0.05)", border: "1px solid rgba(37,211,102,0.1)", borderRadius: 16, padding: 20 }}>
                                                        <div style={{ fontSize: 11, fontWeight: 900, color: "#25D366", marginBottom: 12 }}>🚀 RECOMENDADA (EL CIERRE)</div>
                                                        <div style={{ fontSize: 14, color: "#E5E7EB", lineHeight: 1.6, marginBottom: 16 }}>{waAnalysisResult.responses.recommended}</div>
                                                        <button onClick={() => { navigator.clipboard.writeText(waAnalysisResult.responses.recommended); setToast({ msg: "Copiada", type: 'success' }); }} style={{ background: "#25D366", color: "#000", border: "none", padding: "8px 16px", borderRadius: 8, fontWeight: 800, fontSize: 12, cursor: "pointer" }}>Copiar</button>
                                                    </div>
                                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                                                        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 16, padding: 20 }}>
                                                            <div style={{ fontSize: 11, fontWeight: 900, color: "#9CA3AF", marginBottom: 12 }}>SUAVE</div>
                                                            <div style={{ fontSize: 13, color: "#9CA3AF", lineHeight: 1.6, marginBottom: 16, minHeight: 60 }}>{waAnalysisResult.responses.soft}</div>
                                                            <button onClick={() => { navigator.clipboard.writeText(waAnalysisResult.responses.soft); setToast({ msg: "Copiada", type: 'success' }); }} style={{ background: "rgba(255,255,255,0.1)", color: "#fff", border: "none", padding: "8px 16px", borderRadius: 8, fontWeight: 800, fontSize: 11, cursor: "pointer" }}>Copiar</button>
                                                        </div>
                                                        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 16, padding: 20 }}>
                                                            <div style={{ fontSize: 11, fontWeight: 900, color: "#9CA3AF", marginBottom: 12 }}>CIERRE DIRECTO</div>
                                                            <div style={{ fontSize: 13, color: "#9CA3AF", lineHeight: 1.6, marginBottom: 16, minHeight: 60 }}>{waAnalysisResult.responses.direct}</div>
                                                            <button onClick={() => { navigator.clipboard.writeText(waAnalysisResult.responses.direct); setToast({ msg: "Copiada", type: 'success' }); }} style={{ background: "rgba(255,255,255,0.1)", color: "#fff", border: "none", padding: "8px 16px", borderRadius: 8, fontWeight: 800, fontSize: 11, cursor: "pointer" }}>Copiar</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
                                                <div className="glass-card" style={{ padding: 24 }}>
                                                    <div style={{ display: "flex", alignContent: "center", gap: 10, marginBottom: 16, color: "#facc15" }}>
                                                        <ShieldAlert size={18} /> <span style={{ fontWeight: 800, fontSize: 14 }}>ALERTAS</span>
                                                    </div>
                                                    <ul style={{ margin: 0, paddingLeft: 20, fontSize: 13, color: "#9CA3AF", display: "flex", flexDirection: "column", gap: 10 }}>
                                                        {waAnalysisResult.alerts.map((a: string, i: number) => <li key={i}>{a}</li>)}
                                                    </ul>
                                                </div>
                                                <div className="glass-card" style={{ padding: 24 }}>
                                                    <div style={{ display: "flex", alignContent: "center", gap: 10, marginBottom: 16, color: "#A78BFA" }}>
                                                        <TrendingUp size={18} /> <span style={{ fontWeight: 800, fontSize: 14 }}>MEJORA CLAVE</span>
                                                    </div>
                                                    <p style={{ margin: 0, fontSize: 13, color: "#E5E7EB", lineHeight: 1.5 }}>{waAnalysisResult.improvement}</p>
                                                </div>
                                            </div>

                                            <div className="glass-card" style={{ padding: 32 }}>
                                                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
                                                    <h3 style={{ fontSize: 18, fontWeight: 900, display: "flex", alignItems: "center", gap: 10, margin: 0 }}><Layers size={20} /> Flujo Recomendado</h3>
                                                    <div style={{ fontSize: 12, fontWeight: 800, color: "#25D366", background: "rgba(37,211,102,0.1)", padding: "4px 12px", borderRadius: 20 }}>SEGUIMIENTO DINÁMICO</div>
                                                </div>
                                                <div style={{ display: "flex", gap: 24 }}>
                                                    {waAnalysisResult.flow.map((f: string, i: number) => (
                                                        <div key={i} style={{ flex: 1, padding: 16, background: "rgba(255,255,255,0.02)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.05)" }}>
                                                            <div style={{ fontSize: 10, color: "#6B7280", marginBottom: 8, fontWeight: 800 }}>PASO {i + 1}</div>
                                                            <div style={{ fontSize: 13, color: "#D1D5DB" }}>{f}</div>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                            ...
                                            <div className="glass-card" style={{ padding: 32 }}>
                                                <h3 style={{ fontSize: 18, fontWeight: 900, marginBottom: 24, display: "flex", alignItems: "center", gap: 10 }}><Calendar size={20} /> Seguimientos Automáticos</h3>
                                                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 20 }}>
                                                    <div style={{ background: "rgba(255,255,255,0.03)", padding: 16, borderRadius: 16 }}>
                                                        <div style={{ fontSize: 10, fontWeight: 800, color: "#9CA3AF", marginBottom: 12 }}>A LAS 24H</div>
                                                        <p style={{ fontSize: 12, color: "#9CA3AF", marginBottom: 12, minHeight: 40 }}>{waAnalysisResult.followUps.h24}</p>
                                                        <button onClick={() => { navigator.clipboard.writeText(waAnalysisResult.followUps.h24); setToast({ msg: "Copiada", type: 'success' }); }} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", padding: "6px 12px", borderRadius: 8, fontSize: 10, cursor: "pointer" }}>Copiar</button>
                                                    </div>
                                                    <div style={{ background: "rgba(255,255,255,0.03)", padding: 16, borderRadius: 16 }}>
                                                        <div style={{ fontSize: 10, fontWeight: 800, color: "#9CA3AF", marginBottom: 12 }}>SI DEJÓ EN VISTO</div>
                                                        <p style={{ fontSize: 12, color: "#9CA3AF", marginBottom: 12, minHeight: 40 }}>{waAnalysisResult.followUps.seen}</p>
                                                        <button onClick={() => { navigator.clipboard.writeText(waAnalysisResult.followUps.seen); setToast({ msg: "Copiada", type: 'success' }); }} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", padding: "6px 12px", borderRadius: 8, fontSize: 10, cursor: "pointer" }}>Copiar</button>
                                                    </div>
                                                    <div style={{ background: "rgba(255,255,255,0.03)", padding: 16, borderRadius: 16 }}>
                                                        <div style={{ fontSize: 10, fontWeight: 800, color: "#9CA3AF", marginBottom: 12 }}>DIJO "LO PENSARÉ"</div>
                                                        <p style={{ fontSize: 12, color: "#9CA3AF", marginBottom: 12, minHeight: 40 }}>{waAnalysisResult.followUps.think}</p>
                                                        <button onClick={() => { navigator.clipboard.writeText(waAnalysisResult.followUps.think); setToast({ msg: "Copiada", type: 'success' }); }} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", padding: "6px 12px", borderRadius: 8, fontSize: 10, cursor: "pointer" }}>Copiar</button>
                                                    </div>
                                                </div>
                                            </div>

                                            <div className="glass-card" style={{ padding: 32 }}>
                                                <h3 style={{ fontSize: 18, fontWeight: 900, marginBottom: 24, display: "flex", alignItems: "center", gap: 10, color: "#facc15" }}><Bookmark size={20} /> Biblioteca de Cierres Rápidos</h3>
                                                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                                                    {LIBRARY_OF_CLOSURES.map((c, i) => (
                                                        <div key={i} style={{ background: "rgba(250,204,21,0.03)", border: "1px solid rgba(250,204,21,0.1)", padding: 16, borderRadius: 16 }}>
                                                            <div style={{ fontSize: 10, fontWeight: 900, color: "#facc15", marginBottom: 8, textTransform: "uppercase" }}>{c.cat}</div>
                                                            <p style={{ fontSize: 12, color: "#D1D5DB", marginBottom: 12, lineHeight: 1.4 }}>{c.text}</p>
                                                            <button onClick={() => { navigator.clipboard.writeText(c.text); setToast({ msg: "Copiado", type: 'success' }); }} style={{ background: "rgba(250,204,21,0.1)", border: "1px solid rgba(250,204,21,0.2)", color: "#facc15", padding: "6px 12px", borderRadius: 8, fontSize: 10, cursor: "pointer", fontWeight: 700 }}>Copiar Cierre</button>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: waSimEvaluation ? "1fr" : "1fr 1fr", gap: 32 }}>
                                {!waSimEvaluation && (
                                    <div className="glass-card" style={{ padding: 32, display: "flex", flexDirection: "column", gap: 20 }}>
                                        <h3 style={{ fontSize: 18, fontWeight: 900, marginBottom: 10 }}>Entrenamiento de Closer</h3>
                                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                                            <div>
                                                <label style={{ fontSize: 11, fontWeight: 800, color: "#25D366", marginBottom: 10, display: "block" }}>TIPO DE CLIENTE</label>
                                                <select
                                                    value={waClientType}
                                                    onChange={(e) => { setWaClientType(e.target.value); setWaSimMessages([]); setWaSimEvaluation(null); }}
                                                    className="input-field"
                                                    style={{ padding: "12px 16px" }}
                                                >
                                                    <option value="cliente frío (solo pregunta precio)">Cliente Frío (Solo Precio)</option>
                                                    <option value="cliente desconfiado (cree que es estafa o no funciona)">Cliente Desconfiado</option>
                                                    <option value="cliente indeciso (le gusta pero no sabe cuándo empezar)">Cliente Indeciso</option>
                                                    <option value="cliente listo para comprar (quiere los detalles finales)">Cliente Caliente</option>
                                                    <option value="cliente que objeta por falta de dinero">Objeción: No tengo dinero</option>
                                                </select>
                                            </div>
                                            <div>
                                                <label style={{ fontSize: 11, fontWeight: 800, color: "#25D366", marginBottom: 10, display: "block" }}>PRODUCTO / PROYECTO</label>
                                                <select
                                        ...
                                        {activeProject && (
                                            <div style={{ background: "rgba(37,211,102,0.05)", border: "1px solid rgba(37,211,102,0.1)", padding: "14px 20px", borderRadius: 16 }}>
                                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                                    <div>
                                                        <div style={{ fontSize: 10, color: "#25D366", fontWeight: 800, marginBottom: 4 }}>PRODUCTO ACTIVO</div>
                                                        <div style={{ fontSize: 13, fontWeight: 700 }}>{activeProject.productName}</div>
                                                    </div>
                                                    <div style={{ textAlign: "right" }}>
                                                        <div style={{ fontSize: 10, color: "#9CA3AF", fontWeight: 800, marginBottom: 4 }}>AVATAR (AUDIENCIA)</div>
                                                        <div style={{ fontSize: 12, color: "#9CA3AF" }}>{activeProject.targetAudience}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        )}

                                        <p style={{ fontSize: 13, color: "#9CA3AF", margin: 0 }}>Interactúa con la IA como si fuera un cliente real. Al terminar, pide tu evaluación.</p>

                                        <div style={{ flex: 1, minHeight: 400, background: "rgba(0,0,0,0.3)", borderRadius: 20, padding: 24, overflowY: "auto", display: "flex", flexDirection: "column", gap: 16 }} className="custom-scrollbar">
                                            {waSimMessages.length === 0 && (
                                                <div style={{ textAlign: "center", opacity: 0.6, marginTop: 100, display: "flex", flexDirection: "column", alignItems: "center" }}>
                                                    <MessageSquare size={48} style={{ marginBottom: 16, color: "#25D366" }} />
                                                    <p style={{ marginBottom: 20, fontWeight: 600 }}>La IA actuará como un cliente interesado.</p>
                                                    <button
                                                        onClick={handleWaSimStart}
                                                        disabled={isWaSimLoading}
                                                        style={{ background: "#25D366", color: "#000", border: "none", padding: "12px 24px", borderRadius: 12, fontWeight: 900, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}
                                                    >
                                                        {isWaSimLoading ? <Loader2 className="animate-spin" /> : <><Sparkles size={16} /> Iniciar Conversación</>}
                                                    </button>
                                                </div>
                                            )}
                                            {waSimMessages.map((m, i) => (
                                                <div key={i} style={{ alignSelf: m.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: "80%", background: m.role === 'user' ? "#25D366" : "rgba(255,255,255,0.05)", color: m.role === 'user' ? "#000" : "#fff", padding: "12px 18px", borderRadius: m.role === 'user' ? "20px 20px 4px 20px" : "20px 20px 20px 4px", fontSize: 14, fontWeight: m.role === 'user' ? 600 : 400 }}>
                                                    {m.content}
                                                </div>
                                            ))}
                                            {isWaSimLoading && <div style={{ alignSelf: "flex-start", background: "rgba(255,255,255,0.05)", padding: "12px 18px", borderRadius: "20px 20px 20px 4px" }}><Loader2 className="animate-spin" size={16} /></div>}
                                        </div>
                                        <div style={{ display: "flex", gap: 12 }}>
                                            <input
                                                type="text"
                                                ...
                                                onKeyPress={(e) => e.key === 'Enter' && handleWaSimSendMessage()}
                                                placeholder="Escribe tu mensaje aquí..."
                                                className="input-field"
                                                style={{ flex: 1 }}
                                            />
                                            <button onClick={handleWaSimSendMessage} disabled={isWaSimLoading} style={{ background: "#25D366", border: "none", width: 50, height: 50, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#000" }}><Send size={20} /></button>
                                        </div>
                                        {waSimMessages.length > 2 && (
                                            <button
                                                onClick={handleWaSimEvaluate}
                                                disabled={isEvaluatingWa}
                                                style={{ width: "100%", background: "transparent", border: "1px dashed rgba(37,211,102,0.3)", color: "#25D366", padding: 14, borderRadius: 16, cursor: "pointer", fontWeight: 700, fontSize: 13, marginTop: 10 }}
                                            >
                                                {isEvaluatingWa ? <Loader2 className="animate-spin" /> : "Terminar y Evaluar Desempeño"}
                                            </button>
                                        )}
                                    </div>
                                )}
                                ...
                                {!waSimEvaluation && (
                                    <div className="glass-card" style={{ padding: 40, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
                                        <Brain size={64} style={{ marginBottom: 24, color: "#25D366" }} />
                                        <h2 style={{ fontSize: 24, fontWeight: 900, marginBottom: 16 }}>Modo Entrenamiento</h2>
                                        <p style={{ color: "#9CA3AF", lineHeight: 1.6, maxWidth: 400 }}>Recibe feedback instantáneo sobre tu capacidad de cierre. Aprende a manejar objeciones difíciles en un entorno seguro.</p>
                                        <div style={{ marginTop: 40, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, width: "100%" }}>
                                            <div style={{ background: "rgba(255,255,255,0.03)", padding: 20, borderRadius: 20, border: "1px solid rgba(255,255,255,0.05)" }}>
                                                <CheckCircle2 size={24} style={{ color: "#25D366", marginBottom: 12 }} />
                                                <div style={{ fontSize: 14, fontWeight: 800 }}>Feedback Real</div>
                                            </div>
                                            <div style={{ background: "rgba(255,255,255,0.03)", padding: 20, borderRadius: 20, border: "1px solid rgba(255,255,255,0.05)" }}>
                                                <Zap size={24} style={{ color: "#facc15", marginBottom: 12 }} />
                                                <div style={{ fontSize: 14, fontWeight: 800 }}>Guion Experto</div>
                                            </div>
                                        </div>
                                    </div>
                                )}
                                ...
                                {waSimEvaluation && (
                                    <div className="anim-in" style={{ display: "flex", flexDirection: "column", gap: 32 }}>
                                        <div className="glass-card" style={{ padding: 40, textAlign: "center" }}>
                                            <div style={{ background: "rgba(37,211,102,0.1)", width: 120, height: 120, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 32px" }}>
                                                <div style={{ fontSize: 36, fontWeight: 900, color: "#25D366" }}>{waSimEvaluation.score.overall}</div>
                                            </div>
                                            <h2 style={{ fontSize: 32, fontWeight: 900, marginBottom: 8 }}>Evaluación de Closer</h2>
                                            <p style={{ color: "#9CA3AF", fontSize: 16 }}>Has completado la simulación con un perfil: {waClientType}</p>
                                        </div>

                                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 24 }}>
                                            <div className="glass-card" style={{ padding: 24, textAlign: "center" }}>
                                                <div style={{ fontSize: 10, color: "#9CA3AF", marginBottom: 8, fontWeight: 800 }}>CLARIDAD</div>
                                                <div style={{ fontSize: 18, color: "#25D366", fontWeight: 900 }}>{waSimEvaluation.score.clarity}%</div>
                                            </div>
                                            <div className="glass-card" style={{ padding: 24, textAlign: "center" }}>
                                                <div style={{ fontSize: 10, color: "#9CA3AF", marginBottom: 8, fontWeight: 800 }}>OBJECIONES</div>
                                                <div style={{ fontSize: 18, color: "#25D366", fontWeight: 900 }}>{waSimEvaluation.score.objections}%</div>
                                            </div>
                                            <div className="glass-card" style={{ padding: 24, textAlign: "center" }}>
                                                <div style={{ fontSize: 10, color: "#9CA3AF", marginBottom: 8, fontWeight: 800 }}>CIERRE</div>
                                                <div style={{ fontSize: 18, color: "#25D366", fontWeight: 900 }}>{waSimEvaluation.score.closing}%</div>
                                            </div>
                                        </div>

                                        <div className="glass-card" style={{ padding: 40 }}>
                                            <h3 style={{ fontSize: 18, fontWeight: 900, marginBottom: 32, display: "flex", alignItems: "center", gap: 10 }}><ThumbsUp size={20} /> Análisis de Desempeño</h3>
                                            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                                                <div style={{ background: "rgba(37,211,102,0.05)", border: "1px solid rgba(37,211,102,0.1)", borderRadius: 20, padding: 24 }}>
                                                    <div style={{ fontSize: 12, fontWeight: 900, color: "#25D366", marginBottom: 12 }}>LO QUE HICISTE BIEN</div>
                                                    <p style={{ margin: 0, fontSize: 14, color: "#E5E7EB", lineHeight: 1.6 }}>{waSimEvaluation.feedback.good}</p>
                                                </div>
                                                <div style={{ background: "rgba(239,68,68,0.05)", border: "1px solid rgba(239,68,68,0.1)", borderRadius: 20, padding: 24 }}>
                                                    <div style={{ fontSize: 12, fontWeight: 900, color: "#ef4444", marginBottom: 12 }}>LO QUE HICISTE MAL</div>
                                                    <p style={{ margin: 0, fontSize: 14, color: "#E5E7EB", lineHeight: 1.6 }}>{waSimEvaluation.feedback.bad}</p>
                                                </div>
                                                <div style={{ background: "rgba(139,92,246,0.05)", border: "1px solid rgba(139,92,246,0.1)", borderRadius: 20, padding: 24 }}>
                                                    <div style={{ fontSize: 12, fontWeight: 900, color: "#A78BFA", marginBottom: 12 }}>MÉTODO DE MEJORA</div>
                                                    <p style={{ margin: 0, fontSize: 14, color: "#E5E7EB", lineHeight: 1.6 }}>{waSimEvaluation.feedback.better}</p>
                                                </div>
                                            </div>
                                        </div>

                                        <div className="glass-card" style={{ padding: 48, background: "linear-gradient(135deg, rgba(37,211,102,0.1) 0%, rgba(139,92,246,0.1) 100%)", border: "1px solid rgba(255,255,255,0.1)" }}>
                                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
                                                <h3 style={{ fontSize: 20, fontWeight: 900, margin: 0, display: "flex", alignItems: "center", gap: 12 }}>
                                                    <Award size={24} color="#facc15" /> Así habría respondido un Experto
                                                </h3>
                                                <button onClick={() => { navigator.clipboard.writeText(waSimEvaluation.expertResponse); setToast({ msg: "Copiado", type: 'success' }); }} style={{ background: "#000", color: "#fff", border: "none", padding: "10px 20px", borderRadius: 12, fontWeight: 800, fontSize: 13, cursor: "pointer" }}>Copiar Guion Experto</button>
                                            </div>
                                            <div style={{ background: "rgba(0,0,0,0.3)", padding: 32, borderRadius: 24, fontSize: 16, lineHeight: 1.8, color: "#fff", fontStyle: "italic", border: "1px solid rgba(255,255,255,0.05)" }}>
                                                "{waSimEvaluation.expertResponse}"
                                            </div>
                                        </div>

                                        <button
                                            onClick={() => { setWaSimMessages([]); setWaSimEvaluation(null); }}
                                            className="btn-primary"
                                            style={{ width: "fit-content", alignSelf: "center", background: "transparent", border: "1px solid rgba(255,255,255,0.1)", color: "#9CA3AF" }}
                                        >
                                            <Plus size={18} /> Iniciar Nuevo Entrenamiento
                                        </button>
                                    </div>
                                )}
                            </div>
