...
export default function Dashboard() {
    ...
    const handleGenerate = async (angleId?: string, count: number = 1) => {
        ...
        try {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const finalPrompt = `Create a high quality commercial ad image. Professional photography style: ${selectedAngle.style}. AD CREATIVE OBJECTIVE: ${selectedAngle.goal}. ${brandingContext} ${customContext} DIMENSIONS: ${selectedOutputSize || '1:1'}. Variation ${i + 1}`;
    ...
    const handleGenerateLanding = async () => {
        ...
        try {
            ...
            const prompt = `Crea un gráfico vertical (447x800) para una landing page. CATEGORÍA: ${landingCategory}. IDIOMA: ${language}. ${specificPrompt} ${customContext} ${colorStyle} ORTOGRAFÍA PERFECTA EN ${language}. CALIDAD 8K.`;
    ...
    const handleRefine = async (index: number, isLanding = false) => {
        ...
        try {
            const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-image-preview:generateContent?key=${apiKey}`;
            const base64Data = prevImage.includes(",") ? prevImage.split(",")[1] : prevImage;
                                        ...
                                        {landingResults ? (
                                            <div style={{ position: "relative" }}>
                                                <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
                                                    ...
                                                    <button
                                                        onClick={() => saveToLibrary(landingResults, landingCategory)}
                                                        className="btn-primary"
                                                    ...
                                                    >
                                                        <Bookmark size={18} fill={library.some(a => a.image === landingResults) ? (activeProject?.primaryColor || "#8B5CF6") : "none"} />
                                                    </button>
                                                </div>
