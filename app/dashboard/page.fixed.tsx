...
export default function Dashboard() {
    ...
    useEffect(() => {
        const savedUser = localStorage.getItem("adstools_user");
        if (!savedUser) {
            router.push("/login");
        } else {
            try {
                ...
                localStorage.removeItem("adstools_user");
                router.push("/login");
            }
        }

        // Admin Mode state recovery
        const adminStatus = localStorage.getItem(getUKey("adstools_admin_mode")) === 'true';
    ...
    const [logoPrimaryColor, setLogoPrimaryColor] = useState("#EF4444");
    ...
    useEffect(() => {
        if (!user) return;
        const streakKey = getUKey('adstools_streak');
        const weekKey = getUKey('adstools_week_activity');
    ...
    useEffect(() => {
        ...
        const savedKey = localStorage.getItem(getUKey("adstools_api_key")) || localStorage.getItem("adstools_api_key_global");
        if (savedKey) {
            setApiKey(savedKey);
        ...
        const loadProjects = async () => {
            ...
            if (idbProjects && idbProjects.length > 0) {
                ...
                const savedProjects = localStorage.getItem(getUKey("adstools_projects"));
                const legacyProjects = localStorage.getItem("adstools_projects");
        ...
        const loadLib = async () => {
            ...
            if (idbLib && idbLib.length > 0) {
                ...
                const savedLib = localStorage.getItem(getUKey("adstools_library"));
                if (savedLib) {
                    try {
        ...
        const savedPhoto = localStorage.getItem(getUKey("adstools_user_photo")) || (user as any)?.photo;
        if (savedPhoto) setUserPhoto(savedPhoto);
        else setUserPhoto(null);

        const savedPosts = localStorage.getItem("adstools_community_posts");
        if (savedPosts) {
    ...
    const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (event) => {
            ...
            setUserPhoto(res);
            localStorage.setItem(getUKey("adstools_user_photo"), res);
    ...
    const toggleAdmin = () => {
        if (secretKey === "ADSTOOLS2025") { // Llave secreta para activar modo admin
            const newAdminStatus = !isAdmin;
            setIsAdmin(newAdminStatus);
            localStorage.setItem(getUKey("adstools_admin_mode"), newAdminStatus.toString());
    ...
    const deletePost = (id: string) => {
        ...
        setPosts(newPosts);
        localStorage.setItem("adstools_community_posts", JSON.stringify(newPosts));
    ...
    const [selectedPrimary, setSelectedPrimary] = useState("#EF4444");
    ...
    const removeFromLibrary = (id: string) => {
        ...
        setLibrary(newLib);
        localStorage.setItem(getUKey("adstools_library"), JSON.stringify(newLib));
    };
    ...
    const handlePost = () => {
        ...
        const newPost: Post = {
            id: Math.random().toString(36).substr(2, 9),
            author: user?.name || "Admin AdsTools",
            authorAvatar: getAvatarUrl(),
            category: postingTo,
        ...
        localStorage.setItem("adstools_community_posts", JSON.stringify(updatedPosts));
        setNewPostContent("");
    ...
    const handleGenerateLanding = async () => {
        ...
        try {
            ...
            const colorStyle = `Color Primario: ${activeProject.primaryColor || '#EF4444'}, Secundario: ${activeProject.secondaryColor || '#FFFFFF'}, Fuente: ${activeProject.font || 'Inter'}.`;
    ...
    return (
        <div style={{ background: "#030303", minHeight: "100vh", color: "#fff", fontFamily: "Inter, sans-serif", display: "flex" }}>
            <style>{`
                ...
                .btn-primary { background: #EF4444; color: #fff; border: none; padding: 16px 32px; border-radius: 16px; font-weight: 800; cursor: pointer; transition: 0.2s; display: flex; align-items: center; gap: 10px; }
                .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 10px 30px rgba(239, 68, 68, 0.4); }
                ...
                .nav-item.active { background: rgba(239, 68, 68,0.08); border-color: rgba(239, 68, 68,0.15); color: #EF4444; }
                ...
                .project-card:hover { border-color: #EF4444; background: rgba(255,255,255,0.04); }
                ...
                .community-cat.active { background: #EF4444; color: #fff; }
                ...
                .home-stat:hover { border-color: rgba(239, 68, 68,0.25); background: rgba(239, 68, 68,0.04); }
                ...
                .home-tool-card:hover { border-color: rgba(239, 68, 68,0.3); background: rgba(239, 68, 68,0.04); transform: translateY(-2px); }
                ...
                .btn-purple { background: #EF4444; color: #fff; border: none; padding: 14px 24px; border-radius: 12px; font-weight: 800; cursor: pointer; transition: 0.2s; width: 100%; font-size: 14px; }
                .btn-purple:hover { background: #DC2626; transform: scale(1.02); }
                .progress-bar { height: 6px; background: rgba(255,255,255,0.05); border-radius: 10px; overflow: hidden; position: relative; }
                .progress-fill { height: 100%; background: #EF4444; border: 1px solid rgba(255,255,255,0.05); }
            ...
            <aside style={{ width: 280, borderRight: "1px solid rgba(255,255,255,0.05)", padding: "40px 24px", display: "flex", flexDirection: "column", height: "100vh", position: "fixed", background: "#050505" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 48, padding: "0 12px" }}>
                    <img src="/logo.png" alt="AdsTools" style={{ width: 34, height: 34, objectFit: "contain", borderRadius: 8 }} />
                    <span style={{ fontSize: 24, fontWeight: 900, letterSpacing: "-1px" }}>AdsTools</span>
                </div>
                ...
                <div style={{ marginTop: "auto", display: "flex", flexDirection: "column", gap: 16 }}>
                    ...
                    <button
                        onClick={() => { localStorage.removeItem("adstools_user"); router.push("/login"); }}
            ...
            <main style={{ marginLeft: 300, flex: 1, padding: "40px 60px", overflowX: 'hidden' }}>
                    ...
                    <div style={{ maxWidth: 1100 }} className="anim-in">
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 40 }}>
                            <div>
                                ...
                                <p style={{ color: "#9CA3AF" }}>Este es tu resumen de actividad en AdsTools.</p>
                            </div>
                            <div style={{ display: "flex", gap: 16 }}>
                                <div className="home-stat">
                                    <div style={{ fontSize: 10, fontWeight: 900, color: "#EF4444", textTransform: "uppercase" }}>Imágenes Generadas</div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 32 }}>
                            <div className="glass-card" style={{ padding: 32 }}>
                                <h2 style={{ fontSize: 20, fontWeight: 900, marginBottom: 24, display: "flex", alignItems: "center", gap: 12 }}>
                                    <PlayCircle size={24} color="#EF4444" /> Clases Disponibles
                                </h2>
                            ...
                            <div className="glass-card" style={{ padding: 32, textAlign: "center" }}>
                                ...
                                <div style={{ marginBottom: 24, position: "relative", display: "inline-block" }}>
                                    <div style={{ width: 140, height: 140, borderRadius: "50%", background: "rgba(255,255,255,0.02)", border: "2px solid rgba(239, 68, 68, 0.2)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", margin: "0 auto" }}>
                                        {userPhoto ? (
                                    ...
                                    <label style={{ position: "absolute", bottom: 5, right: 5, background: "#EF4444", color: "#fff", width: 36, height: 36, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", border: "2px solid #030303", cursor: "pointer" }}>
                                        <Plus size={18} />
                                ...
                                <button
                                    ...
                                    className="btn-primary"
                                    style={{ width: "100%", padding: "12px", fontSize: 13, background: "#EF4444" }}
                                >
                                    Editar Perfil
                                </button>
                            </div>
                        ...
                        <div style={{ marginTop: 32 }} className="glass-card">
                            <div style={{ padding: 32, borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <h2 style={{ fontSize: 20, fontWeight: 900, display: "flex", alignItems: "center", gap: 12 }}>
                                    <Sparkles size={24} color="#EF4444" /> Accesos Rápidos
                                </h2>
                            </div>
                        ...
                        {!activeProjectId ? (
                            <>
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 24 }}>
                                    <div onClick={() => setShowProjectModal(true)} className="project-card" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, border: "2px dashed rgba(239, 68, 68,0.3)" }}>
                                        ...
                                        <div key={project.id} onClick={() => setActiveProjectId(project.id)} className="project-card">
                                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
                                                <div style={{ display: "flex", gap: 4 }}>
                                                    <div style={{ width: 32, height: 32, background: project.primaryColor || "#EF4444", borderRadius: 8 }} />
                                        ...
                                        {(activeProject?.results || []).length > 0 && (
                                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                                                ...
                                                {((activeProject?.results || []).filter(r => !r.isLanding).length) > 1 && (
                                                    <button
                                                        onClick={() => downloadAsZip((activeProject?.results || []).filter(r => !r.isLanding).map((r, idx) => ({ image: r.image, name: `${activeProject?.name || 'proj'}_${idx}` })), "adstools_generador")}
                                                        className="btn-primary"
                                                        style={{ padding: "8px 16px", fontSize: 11, background: "rgba(255,255,255,0.05)", border: `1px solid ${activeProject?.primaryColor || "#EF4444"}`, color: activeProject?.primaryColor || "#EF4444" }}
                                                    >
                                                    ...
                                                    <div key={i} className="glass-card" style={{ padding: 12, position: "relative" }}>
                                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                                                            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                                                                <button className="btn-primary" style={{ padding: "6px 12px", fontSize: 10, background: activeProject?.primaryColor || "#EF4444" }} onClick={() => handleGenerate(res.title, 1)}>Más</button>
                                                            ...
                                                            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                                                                <button onClick={() => downloadSingleImage(res.image, activeProject?.name || "image")} style={{ background: "none", border: "none", color: activeProject?.primaryColor || "#EF4444", cursor: "pointer" }}><Download size={16} /></button>
                                                                <button onClick={() => saveToLibrary(res.image, res.title || "Mix")} style={{ background: "none", border: "none", color: activeProject?.primaryColor || "#EF4444", cursor: "pointer" }}>
                                                                    <Bookmark size={16} fill={library.some(a => a.image === res.image) ? (activeProject?.primaryColor || "#EF4444") : "none"} />
                                                                </button>
                                                                <button onClick={() => setZoomedImage(res.image)} style={{ background: "none", border: "none", color: activeProject?.primaryColor || "#EF4444", cursor: "pointer" }}>
                                                                    <Search size={16} />
                                                                </button>
                                                            </div>
                                                        ...
                                                        {refiningIndex === i && (
                                                            <div className="anim-in" style={{ marginTop: 12, padding: 12, background: "rgba(239, 68, 68,0.1)", borderRadius: 12, border: "1px solid rgba(239, 68, 68,0.2)" }}>
                                                        ...
                                                        {res.copy && (
                                                            <div style={{ marginTop: 12, padding: 12, background: "rgba(255,255,255,0.03)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.05)" }}>
                                                                <div style={{ fontSize: 10, fontWeight: 900, color: activeProject?.primaryColor || "#EF4444", marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                                                    AD COPY SUGERIDO
                                                                    <button
                                                                        onClick={() => {
                    ...
                    <div style={{ maxWidth: 1100 }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40 }}>
                            <div className="glass-card" style={{ padding: 40, height: "fit-content" }}>
                                <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
                                    <div>
                                        <label style={{ display: "block", fontSize: 12, fontWeight: 800, color: "#EF4444", marginBottom: 12 }}>FOTO DEL PRODUCTO (TELÉFONO)</label>
                                        <div style={{ border: "2px dashed rgba(255,255,255,0.1)", borderRadius: 24, height: 260, position: "relative", marginBottom: 24, overflow: "hidden" }}>
                                    ...
                                    <button className="btn-primary" style={{ width: "100%", justifyContent: "center", background: "#EF4444" }} onClick={handleGenerateStudio} disabled={isStudioLoading}>
                            ...
                            <div>
                                    ...
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                                        ...
                                        {studioResults.length > 1 && (
                                            <button
                                                ...
                                                className="btn-primary"
                                                style={{ padding: "8px 16px", fontSize: 11, background: "rgba(255,255,255,0.05)", border: `1px solid #EF4444`, color: "#EF4444" }}
                                            >
                                                <Layers size={14} /> Descargar Todos (.ZIP)
                                            </button>
                                        )}
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                                    {(studioResults || []).map((res, i) => (
                                        <div key={i} className="glass-card" style={{ padding: 12, position: "relative" }}>
                                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                                                <div style={{ fontSize: 10, fontWeight: 800, color: "#EF4444" }}>{res.angle}</div>
                                                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                                                    <button onClick={() => downloadSingleImage(res.image, `PhotoStudio_${i}.jpg`)} style={{ background: "none", border: "none", color: "#EF4444", cursor: "pointer" }}><Download size={16} /></button>
                                                    <button onClick={() => saveToLibrary(res.image, res.angle || "Photo Studio")} style={{ background: "none", border: "none", color: "#EF4444", cursor: "pointer" }}>
                                                        <Bookmark size={16} fill={library.some(a => a.image === res.image) ? "#EF4444" : "none"} />
                                                    </button>
                                                    <button onClick={() => setZoomedImage(res.image)} style={{ background: "none", border: "none", color: "#EF4444", cursor: "pointer" }}>
                                                        <Search size={16} />
                                                    </button>
                                                </div>
                                            </div>
                ...
                {activeTab === 'tutorials' && (
                    <div style={{ padding: "0 20px", maxWidth: "1200px", margin: "0 auto" }}>
                        <h1 style={{ fontSize: 32, fontWeight: 900, marginBottom: 40, borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: 20 }}>Tutoriales de AdsTools</h1>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40 }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr", gap: 60 }}>
                            <div className="glass-card" style={{ padding: 40, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center" }}>
                                <div style={{ marginBottom: 32, position: "relative" }}>
                                    <div style={{ width: 220, height: 220, borderRadius: "50%", background: "rgba(255,255,255,0.05)", border: "2px solid rgba(239, 68, 68, 0.3)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                                        {userPhoto ? (
                                            <img src={userPhoto} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                                        ) : (
                                    ...
                                    <label style={{ position: "absolute", bottom: 10, right: 10, background: "#EF4444", color: "#fff", width: 54, height: 54, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", border: "4px solid #030303", cursor: "pointer", transition: "0.2s" }} className="hover-scale">
                                        <Plus />
                                        <input type="file" hidden accept="image/*" onChange={handlePhotoUpload} />
                                    </label>
                                </div>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
                                <div className="glass-card" style={{ padding: 32 }}>
                                    ...
                                    <p style={{ color: "#9CA3AF", fontSize: 14, lineHeight: 1.6, marginBottom: 24 }}>
                                        Utiliza una foto clara de tu rostro. Esta foto se usará como tu identidad en la comunidad AdsTools y en tu perfil personal.
                                    </p>
                                    ...
                                    <div style={{ display: "flex", gap: 12 }}>
                                        ...
                                        <button
                                            onClick={() => {
                                                ...
                                                localStorage.setItem(getUKey("adstools_api_key"), tempApiKey);
                                                localStorage.setItem("adstools_api_key_global", tempApiKey); // Guardar también global para persistencia total
                                    ...
                                    {isAdmin ? (
                                        <div style={{ background: "rgba(16, 185, 129, 0.1)", border: "1px solid #10B981", padding: 20, borderRadius: 16, color: "#10B981", fontWeight: 800, display: "flex", gap: 12, alignItems: "center" }}>
                                            ...
                                            <button
                                                onClick={() => {
                                                    setIsAdmin(false);
                                                    localStorage.removeItem(getUKey("adstools_admin_mode"));
                                        ...
                                        <>
                                            <p style={{ color: "#9CA3AF", fontSize: 14, lineHeight: 1.6, marginBottom: 20 }}>
                                                Si eres parte del equipo de AdsTools, introduce tu clave secreta para gestionar contenido.
                                            </p>
                                            <div style={{ display: "flex", gap: 12 }}>
                                                <input
                                                    type="password"
                                                ...
                                                <button
                                                    onClick={() => {
                                                        if (secretKey === "ADSTOOLS2025") {
                                                            setIsAdmin(true);
                                                            localStorage.setItem(getUKey("adstools_admin_mode"), "true");
                    ...
                    <div style={{ maxWidth: 1000, margin: "0 auto" }} className="anim-in">
                            ...
                            <div className="glass-card" style={{ padding: 40, maxWidth: 600, margin: "0 auto" }}>
                                <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                                        ...
                                        <input className="input-field" placeholder="Ej: AdsTools Pro, BarberShop..." value={logoBusinessName} onChange={(e) => setLogoBusinessName(e.target.value)} />
                                    </div>
                                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        ...
                        {logoStep === 1 && (
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "start" }} className="anim-in">
                                <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
                                    <div className="glass-card" style={{ padding: 40, border: "2px solid #EF4444", position: "relative", minHeight: 400, display: "flex", alignItems: "center", justifyContent: "center" }}>
                                        {isGeneratingLogo ? (
                                ...
                                <div className="glass-card" style={{ padding: 40, minHeight: 400 }}>
                                    ...
                                    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                                        <div>
                                            <label style={{ display: "block", fontSize: 12, fontWeight: 900, color: "#EF4444", marginBottom: 12, textTransform: "uppercase" }}>¿Qué te gustó de este logo?</label>
                                            <textarea
                                                className="input-field"
                                            ...
                                            <div style={{ position: "relative" }}>
                                                <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
                                                    <button
                                                        ...
                                                        className="btn-primary"
                                                        style={{ flex: 1, padding: "12px 16px", fontSize: 12, background: "rgba(255,255,255,0.05)", border: `1px solid ${activeProject?.primaryColor || "#EF4444"}`, color: activeProject?.primaryColor || "#EF4444" }}
                                                    >
                                                        🎨 Ajustar esta pieza
                                                    </button>
                                                    <button
                                                        ...
                                                        className="btn-primary"
                                                        style={{ padding: "12px 16px", fontSize: 12, background: "rgba(255,255,255,0.05)", border: `1px solid ${activeProject?.primaryColor || "#EF4444"}`, color: activeProject?.primaryColor || "#EF4444" }}
                                                    >
                                                        <Bookmark size={18} fill={library.some(a => a.image === landingResults.image) ? (activeProject?.primaryColor || "#EF4444") : "none"} />
                                                    </button>
                                                    <button
                                                ...
                                                {showLandingRefine && (
                                                    <div className="anim-in" style={{ marginBottom: 24, padding: 24, background: "rgba(239, 68, 68,0.1)", borderRadius: 24, border: "1px solid rgba(239, 68, 68,0.2)" }}>
            ...
            {toast && (
                <div style={{ position: "fixed", bottom: 40, right: 40, background: toast.type === 'error' ? '#EF4444' : '#EF4444', color: '#fff', padding: "16px 32px", borderRadius: 16, fontWeight: 800, boxShadow: "0 10px 40px rgba(0,0,0,0.5)", zIndex: 10000 }}>
                    {toast.msg}
                </div>
            )}
            {zoomedImage && (
                    ...
                    <div style={{ maxWidth: 1200, fontFamily: 'Inter, sans-serif' }} className="anim-in">
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 32 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24 }}>
                                ...
                                <div style={{ background: 'linear-gradient(135deg,rgba(239, 68, 68,0.08),rgba(109,40,217,0.04))', border: '1px solid rgba(239, 68, 68,0.2)', borderRadius: 20, padding: 24 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                                        <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(239, 68, 68,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><DollarSign size={16} color="#EF4444" /></div>
                                        <span style={{ fontWeight: 800, fontSize: 16, color: '#EF4444' }}>Gastos Administrativos</span>
                                    </div>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                                        <div style={{ gridColumn: '1 / -1' }}>
                                        ...
                                        <div>
                                            ...
                                            <div style={{ background: 'rgba(239, 68, 68,0.15)', border: '1px solid rgba(239, 68, 68,0.3)', borderRadius: 10, padding: '10px 14px', fontSize: 14, fontWeight: 800, color: '#F87171' }}>$ {fmt(liveGastoAdmin)}</div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }} className="anim-in">
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: 24 }}>
                                            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 24, padding: 28 }}>
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                                    {[
                                                        { label: 'Producto', value: snap.costoProducto, color: '#EF4444' },
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                                                ...
                                                <div style={{ flex: 1, background: 'linear-gradient(135deg,rgba(239, 68, 68,0.1),rgba(109,40,217,0.05))', border: '1px solid rgba(239, 68, 68,0.2)', borderRadius: 24, padding: 28 }}>
                                                    {!finAIResult ? (
                                                        <button
                                                            onClick={handleGenerateFinAI}
                                                            disabled={isFinAIOn}
                                                            style={{ width: '100%', padding: '16px', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg,#EF4444,#6d28d9)', color: '#fff', fontSize: 13, fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, boxShadow: '0 5px 20px rgba(239, 68, 68,0.3)' }}
                                                        >
